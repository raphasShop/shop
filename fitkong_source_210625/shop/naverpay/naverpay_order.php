...
$query = "show columns from `{$g5['g5_shop_cart_naverpay_table']}` like 's_cart_id' ";
$res = sql_fetch($query);
if (empty($res)) {
    sql_query(" ALTER TABLE `{$g5['g5_shop_cart_naverpay_table']}`
                    ADD `s_cart_id` BIGINT(20) NOT NULL default '0'
                    ; ", true);
}

sql_query("delete from {$g5['g5_shop_cart_naverpay_table']} where s_cart_id = '".$s_cart_id."' and ct_select = 0"); // 2019-06-03 : 초기화
// wetoz : naverpayorder
...
for($i=0; $i<$count; $i++) {
    ...
    // wetoz : naverpayorder
    $comma = '';
    $sql = " INSERT INTO {$g5['g5_shop_cart_naverpay_table']}
                    ( MallManageCode, mb_id, it_id, it_name, it_sc_type, it_sc_method, it_sc_price, it_sc_minimum, it_sc_qty, ct_price, ct_point, ct_point_use, ct_stock_use, ct_option, ct_qty, ct_notax, io_id, io_type, io_price, ct_time, ct_ip, ct_send_cost, ct_select, ct_select_time, s_cart_id )
                VALUES ";
    $ct_count = 0;
    // wetoz : naverpayorder
    ...
    for($k=0; $k<$opt_count; $k++) {
        ...
        if($ct_send_cost != 1) {  //
            ...
            $MALL_MANAGE_CODE = get_uniqid(); // wetoz : naverpayorder
        ...
        $options[$it_id][] = array(
            ...
            'type'      => $io_type,
            'io_id'     => $io_id,
            'MALL_MANAGE_CODE'     => $MALL_MANAGE_CODE, // wetoz : naverpayorder
        );

        // wetoz : naverpayorder
        $sql .= $comma."( '$MALL_MANAGE_CODE', '{$member['mb_id']}', '{$it['it_id']}', '".addslashes($it['it_name'])."', '{$it['it_sc_type']}', '{$it['it_sc_method']}', '{$it['it_sc_price']}', '{$it['it_sc_minimum']}', '{$it['it_sc_qty']}', '{$it['it_price']}', '$point', '0', '0', '$io_value', '$ct_qty', '{$it['it_notax']}', '$io_id', '$io_type', '$io_price', '".G5_TIME_YMDHIS."', '".$_SERVER['REMOTE_ADDR']."', '$ct_send_cost', '$ct_select', '$ct_select_time', '$s_cart_id')";
            $comma = ' , ';
        $ct_count++;
        // wetoz : naverpayorder

        //promotion address : naverpayorder
        promotion_naver_order_log($MALL_MANAGE_CODE);

        $pa_user_code = get_cookie('ck_pa_user_code');
        $pa_code = get_cookie('ck_pa_code');
        $desc = $pa_code.' '.$pa_user_code;
        set_behavior_log($MALL_MANAGE_CODE,'naverpayorder',$desc,$od_id,$od_cart_price);

    }

    if($ct_count > 0) sql_query($sql); // wetoz : naverpayorder
    
    // promotion address : behavior
    if($ct_count > 0) {
       //promotion_naver_order_log($s_cart_id);
       promotion_behavior_log('naverpay');
    }
}
