...
$INIpayLog  = true;                  // 로그를 기록하려면 true 로 수정
...
if( $PG_IP == "203.238.37.3" || $PG_IP == "203.238.37.15" || $PG_IP == "203.238.37.16" || $PG_IP == "203.238.37.25" || $PG_IP == "39.115.212.9" )  //PG에서 보냈는지 IP로 체크
{
        ...
        if($result) {
            ...
            if($row['cnt'] == 1) {
                ...
                $sql2 = " select it_name from {$g5['g5_shop_cart_table']} where od_id = '$od_id' LIMIT 1";
                $od_cart = sql_fetch($sql2);

                $today = date("Y/m/d");
                $od_date = $info['od_time'];
                $dateVal = substr($od_date, 0, 10);
                $kakao_rcv_number = preg_replace("/[^0-9]/", "", $info['od_hp']);

                $msg = "[핏콩]\r\n안녕하세요.".$info['od_name']."님!\r\n \r\n입금 확인되었으며,\r\n곧 배송처리 진행 예정입니다.\r\n \r\n*주문번호: ".$info['od_id']."\r\n*상품명: ".$od_cart['it_name']."\r\n*주문일자: ".$dateVal."\r\n*입금액: ".number_format($amt_input)."원";
                $rst = kakaomsg('http://api.apistore.co.kr/kko/1/msg/fitkong', array('PHONE'=>$kakao_rcv_number, 'CALLBACK'=>'01085808413', 'MSG'=>$msg, 'TEMPLATE_CODE'=>'FK0006'));
