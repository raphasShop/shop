...
if($it['it_7'] == 'member') {
    if (!$member['mb_id'])
    {
        alert('회원만 구매가능합니다.', G5_BBS_URL.'/login.php?url=' . urlencode(G5_SHOP_URL.'/item.php?it_id='.$it_id));
    }
}
...
if($it['it_6']) {
  ...
  for($i=0; $row=sql_fetch_array($res); $i++) {
        if($row['ct_status'] == '입금' || $row['ct_status'] == '배송' || $row['ct_status'] == '완료') {
            $order_cnt = $order_cnt + $row['ct_qty'];
        }  
  }
  if($order_cnt >= (int)$it['it_6']) {
...
// 리뷰 팝업 정보를 얻음
$sql = " select * from {$g5['g5_shop_review_popup_table']} where it_id = '$it_id' ";
$rvp = sql_fetch($sql);
$rvp = shuffle($rvp);
...
?>
    <ul class="sanchor">
        <li><a href="#sit_inf" <?php if ($anc_id == 'inf') echo 'class="sanchor_on"'; ?>>상품 설명</a></li>
        <li><a href="#sit_use" <?php if ($anc_id == 'use') echo 'class="sanchor_on"'; ?>>상품 리뷰 (<span class="item_use_count"><?php echo $item_use_count; ?></span>)</a></li>
        <li><a href="#sit_qa" <?php if ($anc_id == 'qa') echo 'class="sanchor_on"'; ?>>상품 문의</a></li>
        <?php if ($default['de_baesong_content']) { ?><li><a href="#sit_dvr" <?php if ($anc_id == 'dvr') echo 'class="sanchor_on"'; ?>>구매 안내</a></li><?php } ?>
