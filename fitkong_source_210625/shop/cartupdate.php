...
else // 장바구니에 담기
{
    ...
    for($i=0; $i<$count; $i++) {
        ...
        if($promotion_item) {
            ...
            $mb_date = substr($mb_datetime, 0, 10);

            if($mb_date != '2021-04-29' && $mb_date != '2021-04-30' && $mb_date != '2021-05-01' && $mb_date != '2021-05-02' && $mb_date != '2021-05-03') {
                //alert("신규회원만 이벤트 상품 주문이 가능합니다.");
            }

            $mbdate_str = $mb_date;
            $startdate_str = "2021-06-10";

            $startdate = strtotime($startdate_str);
            $mbdate = strtotime($mbdate_str);
            $enddate = strtotime("+3 days", $mbdate);

            $currentdate = strtotime(date('Y-m-d'));

            $startdiff = $mbdate - $startdate;
            $enddiff = $currentdate - $enddate;
            if($startdiff >= 0) {} else {
                alert("신규회원만 이벤트 상품 주문이 가능합니다.");
            }
            if($enddiff > 0) {
                alert("100원 이벤트 상품은 가입 후 3일 내에만 주문 가능합니다.");
            }

            $sql = " select * from {$g5['g5_shop_cart_table']} where mb_id = '$mb_id' ";
            $res = sql_query($sql);
            $order_cnt = 0;
