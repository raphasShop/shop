...
for ($i=0; $row=sql_fetch_array($result); $i++)
{
    ...
    if ((file_exists($bimg) || $bimgurl) && $row['bn_use'] == 1)
    {
        ...
        if($row['bn_main_title'])
            $bimg_class = "banner-slide-back";
        else {
            $bimg_class = "banner-slide-img";
        }
        if($bimgurl)
            echo '<img src="'.$bimgurl.'" alt="'.get_text($row['bn_alt']).'" class="slide-inner '.$bimg_class.'">';
        else
            echo '<img src="'.G5_DATA_URL.'/banner/'.$row['bn_id'].'" width="'.$size[0].'" alt="'.get_text($row['bn_alt']).'" class="slide-inner '.$bimg_class.'">';
        if($banner)
            echo '</a>'.PHP_EOL;
