...
<div id="shop_order_wrap">
    <div class="shop_con_wrap">
        ...
        ob_start();
        ?>
            <div class="sod_top_title">주문상품정보</div>
            <ul class="sod_list">
                <?php
                $tot_point = 0;
                $tot_sell_price = 0;
                ...
                <li class="sod_li">
                    ...
                    <!-- 입금확인요청을 위한 맨마지막 상품번호 저장 - 아이스크림 소스추가 -->
                    <?php
                    // 입금확인요청을 위한 상품번호 1번째 저장 - 아이스크림
                    $it_idx = $row['it_id'];
                    $it_namex = $row['it_name'];
                    ?>
                    <input type="hidden" name="it_idx" id="it_idx" value="<?php echo $it_idx; ?>">
                    <input type="hidden" name="it_namex" id="it_namex" value="<?php echo $it_namex; ?>">​
                    <!-- // --> 
                    ...
                    <div class="li_op_wr">
                        ...
                        <?php if($coupon_unused == 0) { ?>
                        <div class="li_mod" ><?php echo $cp_button; ?></div>
                        <?php } ?>
                        <div class="total_price"><strong><?php echo number_format($sell_price); ?></strong>원</div>
                    </div>

                    <div class="li_prqty" style="display: none;">
                ...
                <?php
                    ...
                    //쿠폰 할인 및 포인트 제외 체크
                    if($row['it_id'] != '1554801627' && $row['it_id'] != '1551578034') {
                        //$coupon_unused = 0;
                        //echo $coupon_unused;
                    }
                } // for 끝
        ...
        <div id="sod_frm">
            ...
            <?php echo $content; ?>
            <div class="sod_top_title">주문하시는 분</div>
            <section id="sod_frm_orderer" >
                ...
                <div class="odf_list">
                    <ul>
                        ...
                        <li>
                            <label for="od_pwd">주문비밀번호<strong class="sound_only"> 필수</strong></label>
            ...
            <div class="sod_top_title">받으시는 분</div>
            <section id="sod_frm_taker">
                ...
                <div class="odf_list">
                    <ul>
                        ...
                        <li style="display: none;">
                            <label for="od_b_tel">전화번호</label>
                            <input type="text" name="od_b_tel" id="od_b_tel"  class="frm_input " maxlength="20">
                        </li>
                        <li>
                            <label for="od_b_hp">핸드폰<strong class="sound_only"> 필수</strong></label>
            ...
            <div class="sod_top_title">결제 정보</div>
            <section id="sod_frm_pay">
                ...
                <div class="odf_tbl">
                    ...
                    <tr>
                        ...
                        <td><?php echo number_format($tot_sell_price); ?> 원</td>
                    </tr>
                    <?php if($it_cp_count > 0) { ?>
                    <tr>
                        <th scope="row">상품쿠폰할인</th>
                        <td><span id="ct_tot_coupon">0 원</span></td>
                    </tr>
                    <?php } ?>  
                    <tr>
                        <th scope="row">배송비</th>
                        <td><?php echo number_format($send_cost); ?> 원</td>
                    </tr>
                    <tr>
                        <th scope="row">적립금</th>
                        <td><?php echo number_format($tot_point); ?> 원</td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td>
                            <button type="button" id="code_coupon_btn" class="cp_btn1">쿠폰등록</button>
                        </td>
                    </tr>
                    <tr>
                        <input type="hidden" name="url" value="<?php echo G5_SHOP_URL; ?>/orderform.php?sw_direct=1">
                        <td colspan="2"><input type="text" name="coupon_code" class="frm_input" id="coupon_code" value="" placeholder="쿠폰코드입력"></td>
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="hidden" name="sc_cp_id" value="">
                            <button type="button" id="point_all_use" class="cp_btn1">전액사용</button>
                        </td>
                    </tr>
                    <?php 
                    $temp_point = 0;
                    ...
                    if ($is_member && $config['cf_use_point'])
                    {
                        ...
                        if ($member['mb_point'] >= $default['de_settle_min_point'])
                        {
                            ...
                            if($coupon_unused == 0) {
                                echo '<tr>';
                                echo '<td colspan="2"><input type="hidden" name="max_temp_point" value="'.$temp_point.'"><input type="text" id="od_temp_point" name="od_temp_point" value="0" class="frm_input" size="10" onKeyup="point_change(this.value);"> 원</td></tr>';
                                echo '<tr><th>보유포인트<br>최대사용가능포인트</th><td>'.display_point($member['mb_point']).'<br><span id="use_max_point">'.display_point($temp_point).'</span></br></td>';
                                echo '</tr>';
                            } else {
                                echo '<input type="hidden" id="od_temp_point" name="od_temp_point" value="0" class="frm_input" size="10">';
                            } 
                            $multi_settle++;
                        }
                    }
                    ...
                    <?php } else { ?>
                    <tr>
                        <td colspan="2" style="text-align: center;background: #fafafa">특별 이벤트 행사기간 중에는 쿠폰 및 적립금을<br>중복 사용하실 수 없습니다.</td>
                    </tr>    
                    <?php } ?>
                    <?php $tot_price = $tot_sell_price + $send_cost; // 총계 = 주문상품금액합계 + 배송비 ?>
                     <tr>
                        <th>추가배송비</th>
                        <td><span id="od_send_cost2">0</span>원<br> (지역에 따라 추가되는 도선료 등의 배송비입니다.)</td>
                    </tr>
                    <!-- 쿠폰 할인 제외 끝 -->
                    
                    <tr class="od_total_price_wrap">
                        <th>총 주문금액</th>
            ...
            <div class="sod_top_title">결제 수단</div>
            <section id="sod_frm_pay_method">
                <?php
                ...
                if ($default['de_iche_use']) {
                    $multi_settle++;
                    echo '<li><input type="radio" id="od_settle_iche" name="od_settle_case" value="계좌이체" '.$checked.'> <label for="od_settle_iche" class="lb_icon iche_icon"><span></span>'.$escrow_title.'실시간계좌이체</label></li>'.PHP_EOL;
                    $checked = '';
                }
                ...
                <div class="privacy_check">
                    ...
                    <label for="agree"><span></span>상기 결제 정보를 확인하였으며, 구매에 동의합니다.</label>
                </div>
            </section>
...
$(function() {
    ...
    */
    ...
    $("#code_coupon_btn").click(function() {
        var $this = $(this);
    ...
    $("#point_all_use").click(function() {
        var $this = $(this);
        $("input[name=od_temp_point]").val(<?php echo $temp_point; ?>);
        calculate_order_price();
    });
});
...
function calculate_total_price()
{
    ...
    <?php if($member['mb_level'] == 5) { ?>
        sell_price = Math.floor((sell_price * 0.5) / 10) * 10; 
     <?php } ?>
...
function pay_approval()
{
    if (document.getElementById('agree')) {
        if (!document.getElementById('agree').checked) {
            alert("구매 동의를 먼저 해주셔야 주문이 진행됩니다.");
            return;
        }
    }
    // 재고체크
...
function orderfield_check(f)
{
    ...
    if (typeof(f.od_pwd) != 'undefined')
    {
        ...
        if( (f.od_pwd.value.length<3) || (f.od_pwd.value.search(/([^A-Za-z0-9]+)/)!=-1) )
            error_field(f.od_pwd, "회원이 아니신 경우 주문서 조회시 필요한 주문비밀번호를 3자리 이상 입력해 주십시오.");
    }
