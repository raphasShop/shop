...
<div id="shop_order_wrap">
    <?php if($_GET['code']) { ?>
    <div class="shop_con_msg_wrap">
        <?php
            $order_price_coupon_use = false;
            if($member['mb_id']){
                $cp_offer = 1;
            }
            if((int)$tot_price >= 50000 && (int)$tot_price < 70000) { 
                $od_value = 50000;
                $cp_value = 5;
            } else if((int)$tot_price >= 70000 && (int)$tot_price < 100000) { 
                $od_value = 70000;
                $cp_value = 10;
            } else if((int)$tot_price >= 100000) { 
                $od_value = 100000;
                $cp_value = 15;
            } else {
                $cp_offer = 0;
        ...
        ?>        
            <div><strong>주문이 정상적으로 완료되었습니다. 아래 가상계좌로 입금을 요청드립니다.</strong></div>
            <div class="sod_fin_bank_limit">
                <div class="bank_account"><?php echo $od['od_bank_account']; ?></div>
                <span class="date_limit">
                <?php 
                    $limitdate = strtotime("+2 days", strtotime($od['od_time']));
                    echo date("Y-m-d", $limitdate);
                ?></span> 까지 <span class="misu_price"><?php echo number_format($od['od_misu']); ?>원</span>을 입금해주세요.
            </div>
            <?php if($cp_offer == 1 && $order_price_coupon_use) { ?>
                <div>주문해주셔서 감사드립니다!<br>입금을 완료하시면, <?php echo number_format($od_value); ?>원 이상 구매고객에게 <strong><?php echo $cp_value; ?>% 할인쿠폰</strong> 을 제공해드립니다.</div>
            <?php } ?>
        <?
            } else {
        ?>
            <div><strong>결제가 정상적으로 완료되었습니다.</strong> <br> 상품이 준비중입니다. 발송 후 알림톡으로 안내드리겠습니다.</div>
            <?php if($cp_offer == 1 && $order_price_coupon_use) { ?>
                <div>주문해주셔서 감사드립니다!<br><?php echo number_format($od_value); ?>원 이상 구매고객에게 <strong> <?php echo $cp_value; ?>% 할인쿠폰 </strong> 을 제공해드립니다.</div>
            <?php } ?>
        <?        
            }
        ?>
    ...
    <div class="shop_con_wrap"> 
        <div id="sod_fin">
            ...
            <section class="sod_fin_list">
                ...
                <ul id="sod_list_inq" class="sod_list">
                    ...
                    for($i=0; $row=sql_fetch_array($result); $i++) {
                        ...
                        // 옵션항목
                        $sql = " select ct_id, it_name, ct_option, ct_qty, ct_price, ct_point, ct_status, io_id, io_type, io_price
                                    from {$g5['g5_shop_cart_table']}
                                    where od_id = '$od_id'
                                      and it_id = '{$row['it_id']}'
                    ...
                    <li class="sod_li">
                        ...
                        <?php
                            ...
                            <div class="li_op_wr">
                                ...
                                <div class="li_opt"><?php echo get_text($opt['ct_option']); ?> <?php echo number_format($opt['ct_qty']); ?> 개</div>
                                ...
                                <div class="li_total">
                                    <span class="total_price total_span"><?php echo number_format($sell_price); ?>원</span>
                                </div>
                            ...
                            <?php
                                ...
                                $st_count1++;
                                if($opt['ct_status'] == '주문' || $opt['ct_status'] == '입금')
                                    $st_count2++;

                                // enp order analytics
                                $enp_item_code[$enp_cart_num] = $row['it_id'];
                                $enp_item_name[$enp_cart_num] = get_text($row['it_name']);
                                $gtag_item_name[$enp_cart_num] = get_text($row['it_name']);
                                $gatg_item_option[$enp_cart_num] = '';
                                $gtag_item_code[$enp_cart_num] = $row['it_id'];
                                if($opt['io_id']) {
                                    $enp_item_name[$enp_cart_num] = $enp_item_name[$enp_cart_num]."/".$opt['io_id'];
                                    $gtag_item_option[$enp_cart_num] = $opt['io_id'];
                                    $sqlx = " select io_no 
                                        from {$g5['g5_shop_item_option_table']}
                                        where it_id = '{$row['it_id']}'
                                          and io_id = '{$opt['io_id']}' ";
                                    $io = sql_fetch($sqlx);
                                    $gtag_item_code[$enp_cart_num] = $gtag_item_code[$enp_cart_num]."F".$io['io_no'];
                                }
                                $enp_item_price[$enp_cart_num] = $opt['ct_price'] + $opt['io_price'];
                                $enp_item_qty[$enp_cart_num] = $opt['ct_qty'];
                                $enp_item_cust_price[$enp_cart_num] = $opt['ct_price'] + $opt['io_price'];
                                $enp_cart_item = $enp_cart_item + $opt['ct_qty'];
                                $enp_cart_num++;
                            ...
                            // 구글 전자상거래 전환 상품 구매 데이터
                            $item[$i]['it_name'] = $row['it_name'];
                            $item[$i]['it_id'] = $row['it_id'];
                            $item[$i]['it_qty'] = $sum['qty'];
                            $item[$i]['it_price'] = $opt_price;
                        ?>
            ...
            <?php $send_cost = $od['od_send_cost'] - $od['od_send_coupon']; ?>
            
            <div id="sod_fin_view">
                <div class="sod_title">결제정보</div>
                <h2>결제/배송 정보</h2>
                <?php
                ...
                <section id="sod_fin_orderer">
                    <div class="sod_title">보내시는 분</div>
                ...
                <section id="sod_fin_receiver">
                    <div class="sod_title">받으시는 분</div>
            ...
            <section id="sod_fin_home">
                <a href="<?php echo G5_URL; ?>"><button type="button">메인으로 돌아가기</button></a>
            </section>
...
?>
<!-- Event snippet for 구매전환스크립트 conversion page -->
<script>
  gtag('event', 'conversion', {
      'send_to': 'AW-768312858/matkCIuZ8_cBEJqMru4C',
...
<script>
    gtag('event', 'purchase', {
      ...
      "value": <?php echo $od_cart_price; ?>,
      "currency": 'KRW',
      "shipping": <?php echo $od_send_cost; ?>,
      "items": [
      <?php 
        for ($i=0; $i<$enp_cart_num; $i++) {?>
        {
          "id": '<?php echo $gtag_item_code[$i]; ?>',
          "name": '<?php echo $gtag_item_name[$i]; ?>',
          "variant": '<?php echo $gtag_item_option[$i]; ?>',
          "quantity": <?php echo $enp_item_qty[$i]; ?>,
...
ENP_VAR.conversion.ordCode= '<?php echo $od_id; ?>';
ENP_VAR.conversion.totalPrice = '<?php echo $od_cart_price; ?>';
ENP_VAR.conversion.totalQty = '<?php echo $enp_cart_item; ?>';

    (function(a,g,e,n,t){a.enp=a.enp||function(){(a.enp.q=a.enp.q||[]).push(arguments)};n=g.createElement(e);n.async=!0;n.defer=!0;n.src="https://cdn.megadata.co.kr/dist/prod/enp_tracker_self_hosted.min.js";t=g.getElementsByTagName(e)[0];t.parentNode.insertBefore(n,t)})(window,document,"script");
enp('create', 'conversion', 'jhyim', { device: '<?php echo $device_type; ?>' }); // W:웹, M: 모바일, B: 반응형
enp('send', 'conversion', 'jhyim');
</script>
<!-- Enliple Tracker End -->
<script type="text/javascript">
      kakaoPixel('5283012127697450968').pageView();
      kakaoPixel('5283012127697450968').purchase({
        total_quantity: "<?php echo $enp_cart_item; ?>", // 주문 내 상품 개수(optional)
        total_price: "<?php echo $od_cart_price; ?>",  // 주문 총 가격(optional)
        currency: "KRW",     // 주문 가격의 화폐 단위(optional, 기본 값은 KRW)
        products: [
            <?php 
            for ($i=0; $i<$enp_cart_num; $i++) {?>
            {  
                id: "<?php echo $enp_item_code[$i]; ?>",
                name: "<?php echo $enp_item_name[$i]; ?>",
                quantity: "<?php echo $enp_item_qty[$i]; ?>",
                price: "<?php echo $enp_item_price[$i]; ?>"
            }<?php if($i<$enp_cart_num-1) { echo ",\n"; }} ?>);
        ],
