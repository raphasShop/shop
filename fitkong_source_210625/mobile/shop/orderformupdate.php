...
//주문서작성시 상품ID 가져오는 소스추가 - 아이스크림
//(gr_shop_order_table에 it_idx, it_namex 추가된 부분)
...
//배송지 정보가 없는 경우, 주문내역을 바탕으로 배송지 정보 업데이트
if(!$member['mb_addr1']) {
    $mb_id = $member['mb_id'];
...
$kakao_rcv_number = preg_replace("/[^0-9]/", "", $od_hp);
...
$sql = " select it_name
            ...
            where od_id = '$od_id'
              LIMIT 1";
$it_n = sql_fetch($sql);
$it_name = $it_n['it_name'];

if($od_settle_case == '가상계좌') {
    $msg = "[핏콩]\r\n안녕하세요.".$od_name."님!\r\n \r\n주문해주셔서 진심으로 감사드리며,\r\n아래의 계좌로 입금요청드립니다.\r\n \r\n* 주문번호:".$od_id."\r\n*상품명:".$it_name."\r\n*주문일자:".$today."\r\n*주문금액:".number_format($od_misu)."원\r\n*입금계좌:".$od_bank_account." 입금자명(".$od_deposit_name.")\r\n*입금만료일:".$last_bank_day."";
$rst = kakaomsg('http://api.apistore.co.kr/kko/1/msg/fitkong', array('PHONE'=>$kakao_rcv_number, 'CALLBACK'=>'01085808413', 'MSG'=>$msg, 'TEMPLATE_CODE'=>'FK0003'));
} else {
    $msg = "[핏콩]\r\n안녕하세요. ".$od_name."님!\r\n소중한 주문이 접수완료 되었습니다.\r\n빠른배송 준비하겠습니다. 감사합니다.\r\n \r\n- 주문일 : ".$today."\r\n- 주문번호: ".$od_id."\r\n- 상품명: ".$it_name."";
$rst = kakaomsg('http://api.apistore.co.kr/kko/1/msg/fitkong', array('PHONE'=>$kakao_rcv_number, 'CALLBACK'=>'01085808413', 'MSG'=>$msg, 'TEMPLATE_CODE'=>'FK0002', 'BTN_TYPES'=>'웹링크', 'BTN_TXTS'=>'핏콩 바로가기', 'BTN_URLS1'=>'http://fitkong.co.kr'));
}
...
// promotion address : 프로모션 주문 로그
promotion_access_order($tot_ct_price,$od_id);
promotion_behavior_log('order');


// 첫주문 쿠폰 발행
if($member['mb_id']) {
    $mb_id = $member['mb_id'];
    $sql5 = " select count(*) as cnt from {$g5['g5_shop_order_table']} where mb_id = '$mb_id' ";
    $row5 = sql_fetch($sql5);
    if($row5['cnt'] == 1) {
        $j = 0;
        $create_coupon = false;
...
if($order_price_coupon_use) {
    if((int)$od_receipt_price >= 50000) {
        do {
            $cp_id = get_coupon_id();
        ...
        if($create_coupon) {
            $cp_subject = '구매고객대상 주문할인 쿠폰';
            $cp_method = 2;
            $cp_target = '';
            $cp_start = G5_TIME_YMD;
            $cp_end = date("Y-m-d", (G5_SERVER_TIME + (86400 * ((int)$default['de_member_reg_coupon_term'] - 30))));
            $cp_type = 1;
            $cp_price = 0;
            if((int)$od_receipt_price >= 50000 & (int)$od_receipt_price < 70000) {
                $cp_price = 5;
            } else if((int)$od_receipt_price >= 70000 & (int)$od_receipt_price < 100000) {
                $cp_price = 10;
            } else if ((int)$od_receipt_price >= 100000 ) {
                $cp_price = 15;
            } else {
                $cp_price = 0; 
            }
            ...
            $sql = " INSERT INTO {$g5['g5_shop_coupon_table']}
                        ( cp_id, cp_subject, cp_method, cp_target, mb_id, cp_start, cp_end, cp_type, cp_price, cp_trunc, cp_minimum, cp_maximum, cp_datetime )
                    VALUES
                        ( '$cp_id', '$cp_subject', '$cp_method', '$cp_target', '{$member['mb_id']}', '$cp_start', '$cp_end', '$cp_type', '$cp_price', '$cp_trunc', '$cp_minimum', '$cp_maximum', '".G5_TIME_YMDHIS."' ) ";
...
goto_url(G5_SHOP_URL.'/orderinquiryview.php?od_id='.$od_id.'&amp;uid='.$uid.'&amp;code=32132');
?>
