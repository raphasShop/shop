...
for ($i=0; $row=sql_fetch_array($result); $i++)
{
    ...
    if ((file_exists($bimg) || $bimgurl) && $row['bn_use'] == 1)
    {
        ...
        echo '<div class="swiper-slide banner-slide">'.PHP_EOL;
        ...
        if($bimgurl)
            echo $banner.'<img src="'.$bimgurl.'" alt="'.get_text($row['bn_alt']).'" class="slide-inner">';
        else
            echo $banner.'<img src="'.G5_DATA_URL.'/banner/'.$row['bn_id'].'" width="'.$size[0].'" alt="'.get_text($row['bn_alt']).'" class="slide-inner">';
        echo '<div class="mainTitle">'.$row['bn_main_title']."</div>";
        echo '<div class="subTitle">'.$row['bn_sub_title']."</div>";
