<?php
$sub_menu = '155703'; /* 수정전 원본 메뉴코드 500500 */
...
$html_title = '핏콩미식회 컨텐츠';
$g5['title'] = $html_title.'관리';

if ($w=="u")
{
    $html_title .= ' 수정';
    $sql = " select * from {$g5['g5_shop_dainty_table']} where co_id = '$co_id' ";
    $co = sql_fetch($sql);
}
else
{
...
<form name="fbanner" action="./daintyformupdate.php" method="post" enctype="multipart/form-data">
...
<div class="tbl_frm01 tbl_wrap">
    ...
    <tr>
        ...
        <td>
            ...
            <?php
            $bimg_str = "";
            $bimg = G5_DATA_PATH."/dainty/{$co['co_id']}";
            if (file_exists($bimg) && $co['co_id']) {
                ...
                $bimg_str = '<img src="'.G5_DATA_URL.'/dainty/'.$co['co_id'].'" width="'.$width.'">';
            }
            if ($bimg_str) {
    ...
    <!--
    <tr>
        <th scope="row"><label for="co_category">카테고리</label></th>
        <td>
            <?php echo help("카테고리명을 입력해주세요. ex)핏콩홈트"); ?>
            <input type="text" name="co_category" value="<?php echo get_text($co['co_category']); ?>" id="co_category" class="frm_input" size="80">
        </td>
    </tr>
    -->
    <tr>
...
<div id="bq_right1">   
    <div class="bq_basic">
    <a href="./daintylist.php"><i class="fas fa-tasks"></i> 목록</a>
    </div>
