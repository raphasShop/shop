...
$sql_common = " from {$g5['g5_shop_instagram_table']} ";
...
<div id="bq_right1">
    ...
    <a href="./instaform.php"><i class="fa fa-plus"></i> 인스타그램컨텐츠추가</a>
    </div>
</div>
<!--//-->
...
<div class="tbl_head02 tbl_wrap">
    ...
    <tr>
        ...
        <th scope="col" id="th_dvc">연결된 URL</th>
    ...
    <tbody>
    <?php
    $sql = " select * from {$g5['g5_shop_instagram_table']}
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++) {
        ...
        // 새창 띄우기인지
        $co_url = $row['co_url'];
        $bimg = G5_DATA_PATH.'/instagram/'.$row['co_id'];
        if(file_exists($bimg)) {
            ...
            $co_img .= '<img src="'.G5_DATA_URL.'/instagram/'.$row['co_id'].'" width="'.$width.'" alt="'.get_text($row['co_alt']).'"></a>';
            $co_thum = '<img src="'.G5_DATA_URL.'/instagram/'.$row['co_id'].'" width="80%" alt="'.get_text($row['co_alt']).'"></a>';
        } else {
            $co_img = "";
            if ($row['co_url'] != "http://")
                $co_img .= '<a href="'.$row['co_url'].'" target="_blank">';
    ...
    <tr class="<?php echo $bg; ?>">
        ...
        <td headers="th_url"><?php echo $co_url; ?></td>
        <td headers="th_odr" class="td_num"><?php echo $row['co_order']; ?></td>
        <td headers="th_use" class="td_num"><?php echo $co_use; ?></td>
        <td headers="th_mng" class="td_mngsmall">
            <a href="./instaform.php?w=u&amp;co_id=<?php echo $row['co_id']; ?>">수정</a></li>
            <a href="./instaformupdate.php?w=d&amp;co_id=<?php echo $row['co_id']; ?>" onclick="return delete_confirm(this);">삭제</a>
        </td>
    </tr>
    <?php
    }
