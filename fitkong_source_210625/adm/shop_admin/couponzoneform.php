...
<div class="tbl_frm01 tbl_wrap">
    ...
    <tr style="display: none;">
        ...
        <td>
           ...
           <select name="cz_type" id="cz_type">
                <option value="0" selected>다운로드쿠폰</option>
           </select>
        </td>
    </tr>
    <tr>
    ...
    <tr>
        <th scope="row"><label for="cz_code">쿠폰코드</label></th>
        <td>
            <input type="text" name="cz_code" value="<?php echo get_text($cp['cz_code']); ?>" id="cz_code" required class="required frm_input" size="50">
        </td>
    </tr>
    <tr>
        <th scope="row"><label for="cz_code_max">쿠폰코드 발행 수</label></th>
        <td>
            <input type="text" name="cz_code_max" value="<?php echo get_text($cp['cz_code_max']); ?>" id="cz_code_max" required class="required frm_input">
        </td>
    </tr>
    <tr>
        <th scope="row"><label for="cz_code_start">쿠폰코드사용시작일</label></th>
        <td>
            <?php echo help('입력 예: '.date('Y-m-d')); ?>
            <input type="text" name="cz_code_start" value="<?php echo stripslashes($cp['cz_code_start']); ?>" id="cz_code_start" required class="frm_input required">
        </td>
    </tr>
    <tr>
        <th scope="row"><label for="cz_code_end">쿠폰코드사용종료일</label></th>
        <td>
            <?php echo help('입력 예: '.date('Y-m-d')); ?>
            <input type="text" name="cz_code_end" value="<?php echo stripslashes($cp['cz_code_end']); ?>" id="cz_code_end" required class="frm_input required">
        </td>
    </tr>
    <tr>
    ...
    <tr>
        ...
        <td>
           ...
           <!-- 쿠폰대상 상품보기 링크 -->
           <?php if($cp['cp_method'] == '0') { //개별상품일때?>
               <button type="button" class="btn btn-skyblue btn-sm" onclick="window.open('<?php echo G5_SHOP_URL; ?>/item.php?it_id=<?php echo $cp['cp_target'];//개별상품?>')">쿠폰이 적용된 상품보기</button><br>
           <?php } else if($cp['cp_method'] == '1') { //개별상품일때?>
               <button type="button" class="btn btn-skyblue btn-sm" onclick="window.open('<?php echo G5_SHOP_URL; ?>/list.php?ca_id=<?php echo $cp['cp_target'];//카테고리?>')">쿠폰이 적용된 상품보기</button><br>
           <?php } //닫기?>
           <!--//-->
...
<div id="bq_right1">   
    <div class="bq_basic">
    <a href="./couponzonelist.php?<?php echo $qstr; ?>"><i class="fas fa-tasks"></i> 목록</a>
    </div>
...
$(function() {
    ...
    $("#cz_start, #cz_end, #cz_code_start, #cz_code_end").datepicker(
...
function form_check(f)
{
    ...
    if(cp_type == "1" && (cp_price < 1 || cp_price > 100)) {
