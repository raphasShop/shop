...
// 관련리뷰를 우선 삭제함
sql_query(" delete from {$g5['g5_shop_item_use_relation_table']} where it_id = '$it_id' ");
...
if ($w == "" || $w == "u")
{
    ...
    // 관련리뷰 등록
    $use_it_id2 = explode(",", $use_it_list);
    for ($i=0; $i<count($use_it_id2); $i++)
    {
        if (trim($use_it_id2[$i]))
        {
            $sql = " insert into {$g5['g5_shop_item_use_relation_table']}
                        set it_id  = '$it_id',
                            it_id2 = '$use_it_id2[$i]',
                            iur_id = '$i' ";
            sql_query($sql, false);
...
<script>
    if (confirm("상품을 계속 등록하시겠습니까?"))
        ...
        location.href = "<?php echo "./itemform.php?wadd=yes&".str_replace('&amp;', '&', $qstr); ?>";
    else
