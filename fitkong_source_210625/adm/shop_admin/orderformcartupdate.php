...
$status_normal = array('주문','입금','준비','배송','완료'); // 교환 추가 - 교환완료시 주문서에 완료로 저장
$status_change = array('교환'); // 교환시 완료로 변경
$status_cancel = array('취소','반품','품절','환불'); // 환불 추가 - 주문서에 취소로 저장

if (in_array($_POST['ct_status'], $status_normal) || in_array($_POST['ct_status'], $status_cancel) || in_array($_POST['ct_status'], $status_change)) {
    ; // 통과
} else {
...
for ($i=0; $i<$cnt; $i++)
{
    ...
    if ($ct['ct_stock_use'])
    {
        if ($ct_status == '주문' || $ct_status == '취소' || $ct_status == '반품' || $ct_status == '품절' || $ct_status == '환불')
        {
            $stock_use = 0;
            // 재고에 다시 더한다.
            if($ct['io_id']) {
    ...
    // it_id를 배열에 저장
    if($ct_status == '주문' || $ct_status == '취소' || $ct_status == '반품' || $ct_status == '품절' || $ct_status == '환불' || $ct_status == '완료')
        $arr_it_id[] = $ct['it_id'];
}
...
if (in_array($_POST['ct_status'], $status_cancel)) {
    $sql = " select count(*) as od_count1,
                    SUM(IF(ct_status = '취소' OR ct_status = '반품' OR ct_status = '품절' OR ct_status = '환불', 1, 0)) as od_count2
...
// 장바구니 상품 모두 교환일 경우 주문상태 완료로 변경 [크림장수 소스추가]
$change_change = false;
if (in_array($_POST['ct_status'], $status_change)) {
    $sql = " select count(*) as od_count1,
                    SUM(IF(ct_status = '교환', 1, 0)) as od_count2
                from {$g5['g5_shop_cart_table']}
                where od_id = '$od_id' ";
    $row = sql_fetch($sql);
...
// 미수금 등의 정보 (lib/shop.lib.php의 1631줄쯔음 function get_order_info($od_id) 을 검색하여 수정)
// 취소,품절,환불시 배송비가 0으로바뀌면서 주문합계가 달라지던것을 배송비가 그대로 있게 수정 [크림장수소스수정]
...
$sql = " update {$g5['g5_shop_order_table']}
            set od_cart_price   = '{$info['od_cart_price']}',
				...
				od_send_cost2   = '{$info['od_send_cost2']}',
...
//$odcancel_price = ($cart_price + $send_cost + $od['od_send_cost2']) - ( $cart_coupon + $od_coupon + $od_send_coupon ); //취소금액(상품가격+배송비-쿠폰할인금액) 
$sql = " select * from {$g5['g5_shop_order_table']} where od_id = '$od_id' ";
$od = sql_fetch($sql);
$odcancel_price = ($od['od_cart_price'] + $od['od_send_cost'] + $od['od_send_cost2']) - ($od['od_cart_coupon'] + $od['od_send_coupon'] + $od['od_coupon'] + $od['od_receipt_point']); //취소금액(상품가격+배송비-쿠폰+결제포인트)
...
$sql = " update {$g5['g5_shop_order_table']}
            set od_cart_price   = '{$info['od_cart_price']}',
                ...
                od_coupon       = '{$info['od_coupon']}',
                od_send_coupon  = '{$info['od_send_coupon']}',
				od_receipt_point= '{$info['od_receipt_point']}',
                od_cancel_price = '$odcancel_price',
                od_misu         = '{$info['od_misu']}',
                od_tax_mny      = '{$info['od_tax_mny']}',
                od_vat_mny      = '{$info['od_vat_mny']}',
                od_free_mny     = '{$info['od_free_mny']}' ";
...
if($cancel_change) {
    $sql .= " , od_status = '취소' "; // 주문상품 모두 취소, 반품, 품절, 환불 이면 주문 취소

} else if($change_change) {
    $sql .= " , od_status = '완료' "; //주문상품 모두 교환인경우 완료로 변경
} else if (in_array($_POST['ct_status'], $status_normal)) { // 정상인 주문상태만 기록   
