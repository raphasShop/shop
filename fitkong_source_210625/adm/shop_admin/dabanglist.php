...
$sql_common = " from {$g5['g5_shop_dabang_table']} ";
...
<div id="bq_right1">
    ...
    <a href="./dabangform.php"><i class="fa fa-plus"></i> 핏콩다방컨텐츠추가</a>
    </div>
</div>
<!--//-->
...
<div class="tbl_head02 tbl_wrap">
    ...
    <tr>
        ...
        <th scope="col" id="th_odr">카테고리</th>
    ...
    <tbody>
    <?php
    $sql = " select * from {$g5['g5_shop_dabang_table']}
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++) {
        ...
        // 새창 띄우기인지
        $co_url = $row['co_url'];
        $bimg = G5_DATA_PATH.'/dabang/'.$row['co_id'];
        if(file_exists($bimg)) {
            ...
            $co_img .= '<img src="'.G5_DATA_URL.'/dabang/'.$row['co_id'].'" width="'.$width.'" alt="'.get_text($row['co_alt']).'"></a>';
            $co_thum = '<img src="'.G5_DATA_URL.'/dabang/'.$row['co_id'].'" width="80%" alt="'.get_text($row['co_alt']).'"></a>';
        } else {
            $co_img = "";
            if ($row['co_url'] != "http://")
                $co_img .= '<a href="'.$row['co_url'].'" target="_blank">';
    ...
    <tr class="<?php echo $bg; ?>">
        ...
        <td headers="th_category" class="td_num"><?php echo $row['co_category']; ?></td>
        ...
        <td headers="th_mng" class="td_mngsmall">
            <a href="./dabangform.php?w=u&amp;co_id=<?php echo $row['co_id']; ?>">수정</a></li>
            <a href="./dabangformupdate.php?w=d&amp;co_id=<?php echo $row['co_id']; ?>" onclick="return delete_confirm(this);">삭제</a>
        </td>
    </tr>
    <?php
    }
