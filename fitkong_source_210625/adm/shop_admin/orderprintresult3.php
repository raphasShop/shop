...
if ($csv == 'csv')
{
    ...
    $sql = " SELECT a.od_receipt_time, a.od_invoice_time, a.od_time, b.io_price, b.od_id, b.mb_id, od_name, b.it_name, b.io_id, b.ct_option, b.ct_qty, b.ct_price, b.cp_price, a.od_cart_price, a.od_send_cost, a.od_send_coupon, a.od_coupon, a.od_receipt_point, b.ct_point, a.od_receipt_price, a.od_settle_case, a.od_status, a.od_pg, a.od_cash
    ...
    echo iconv('utf-8', 'euc-kr', "결제일시,주문일시,주문번호,배송일시,네이버페이주문,주문자ID,주문자명,상품명,옵션명,주문수량,주문품목단가,상품별 총 주문금액,배송비,배송비무료쿠폰,쿠폰할인금액,사용한적립금,적립금,총 결제금액(실결제금액),결제방법,주문상태,현금영수증\n");
...
if ($csv == 'xls')
{
    ...
    $to_date3 = date_conv($to_date3);
     ...
     $sql = " SELECT a.od_receipt_time, a.od_time, b.io_price, b.od_id, b.mb_id, od_name, b.it_name, b.io_id, b.it_id, b.ct_option, b.ct_qty, b.ct_price, b.cp_price, a.od_cart_price, a.od_send_cost, a.od_send_coupon, a.od_coupon, a.od_receipt_point, b.ct_point, a.od_receipt_price, a.od_settle_case, a.od_pg, a.od_status, a.od_cash, a.od_invoice_time 
               FROM {$g5['g5_shop_cart_table']} b, {$g5['g5_shop_order_table']} a
              where b.od_id = a.od_id ";
    $sql .= " and b.ct_status != '쇼핑' ";              
    if ($case == 5) // 출력기간
    ...
    $data = array('결제일시','주문일시','주문번호','배송일시','네이버페이주문','주문자ID','주문자명','상품명','옵션명','주문수량','주문품목단가','상품별 총 주문금액','배송비','배송비무료쿠폰','쿠폰할인금액','사용한적립금','적립금','총 결제금액(실결제금액)','결제방법','주문상태','현금영수증');
    ...
    for($i=1; $row=sql_fetch_array($result); $i++)
    {
        ...
        $worksheet->write($i, 3, $row['od_invoice_time']);//배송일시
        $worksheet->write($i, 4, $io_price);//네이버페이주문
        $worksheet->write($i, 5, $row['mb_id']);//주문자ID
        $worksheet->write($i, 6, $row['od_name']);//주문자명
