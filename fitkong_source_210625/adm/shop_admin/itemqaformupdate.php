...
if ($w == "u")
{
    ...
    // wetoz : naverpayorder
    if ($iq_answer && $default['de_naverpayorder_AccessLicense'] && $default['de_naverpayorder_SecretKey']) {
        $iq = sql_fetch(" select iq_inquiryid, iq_answercontentid from {$g5['g5_shop_item_qa_table']} where iq_id = '$iq_id' ");

        include_once(G5_PLUGIN_PATH.'/wznaverpay/config.php');
        $aor = new NHNAPIORDER();
        $aor->showReq = true;
        $InquiryID = $iq['iq_inquiryid'];
        if ($InquiryID) {
            $AnswerContent = $iq_answer;
            $AnswerContentID = $iq['iq_answercontentid'];
            $ActionType = ($AnswerContentID ? 'UPDATE' : 'INSERT');
            $xml = $aor->AnswerCustomerInquiry($InquiryID, $AnswerContent, $AnswerContentID, $ActionType);
        }
    }
    // wetoz : naverpayorder
