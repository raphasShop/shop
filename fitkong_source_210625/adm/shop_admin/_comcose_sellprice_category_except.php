...
$sql_common = " from comcose_sellprice_category_except ";
$sql_search = " where (1) ";

if ($stx) {$sql_search .= "and (($sfl like '%$stx%'))";}

if (!$sst) {
	$sst  = "cose_except_add_cat_id";
	$sod = "asc";
}
...
<select name="sfl" id="sfl">
    <option value="cose_except_add_cat_caname"<?php echo get_selected($_GET['sfl'], "cose_except_add_cat_caname", true); ?>>분류명</option>
    <option value="cose_except_add_cat_id"<?php echo get_selected($_GET['sfl'], "cose_except_add_cat_id"); ?>>분류ID</option>
</select>
...
<form name="fitemlistupdate" method="post" action="./_comcose_sellprice_category_except_update.php" onsubmit="return fitemlist_submit(this);" autocomplete="off">
...
<div class="tbl_head01 tbl_wrap">
    ...
    <tr>
        ...
        <?php //번호 ?>
        <td style="width:50px;text-align:center">
            <input type="hidden" name="cose_except_cat_id[<?php echo $i; ?>]" value="<?php echo $row['cose_except_cat_id']; ?>">
        ...
        <?php
        $sel_ca_id_sql="SELECT ca_id, ca_name FROM g5_shop_category where ca_id = '{$row['cose_except_add_cat_id']}' order by ca_id ";
        ...
        <?php
        $cat1_id = substr($row['cose_except_add_cat_id'], 0, 2);
        ...
        $cat2_id = substr($row['cose_except_add_cat_id'], 0, 4);
        ...
        $cat3_id = substr($row['cose_except_add_cat_id'], 0, 6);
        ...
        $cat4_id = substr($row['cose_except_add_cat_id'], 0, 8);
        ...
        $cat5_id = substr($row['cose_except_add_cat_id'], 0, 10);
        ...
        $cat_id_level = strlen($row['cose_except_add_cat_id']) / 2;
        $tag = ' ▷ ';
        ...
        <?php //분류ID ?>
        <td style="width:200px;text-align:center">
        <input type="hidden" name="cose_except_add_cat_id[<?php echo $i; ?>]" value="<?php echo $row['cose_except_add_cat_id']; ?>">
        <?php echo $row['cose_except_add_cat_id']; ?>
        </td>
...
<div class="btn_fixed_top">
	<a href="./_comcose_sellprice_category_except_form.php" class="btn btn_01">분류등록</a>
