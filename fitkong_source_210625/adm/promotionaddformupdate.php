...
if($_POST['pa_purl']) {
    $sql = " select count(*) as cnt from {$g5['g5_promotion_address_table']} where pa_code = '$pa_code' ";
    $row = sql_fetch($sql);
    if(!$row['cnt'])
...
if($w == '') {
    ...
    $sql = " INSERT INTO {$g5['g5_promotion_address_table']}
                ( pa_name, pa_code, pa_title, pa_desc, pa_start, pa_end, pa_channel, pa_datetime )
            VALUES
                ( '$pa_name', '$pa_code', '$pa_title', '$pa_desc', '$pa_start', '$pa_end', '$pa_channel', '".G5_TIME_YMDHIS."' ) ";

    sql_query($sql);
} else if($w == 'u') {
    $sql = " select * from {$g5['g5_promotion_address_table']} where pa_id = '$pa_id' ";
    $pa = sql_fetch($sql);
    ...
    $sql = " update {$g5['g5_promotion_address_table']}
                set pa_name  = '$pa_name',
                    pa_code   = '$pa_code',
                    pa_title   = '$pa_title',
                    pa_desc     = '$pa_desc',
