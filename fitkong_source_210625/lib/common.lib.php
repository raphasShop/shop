...
// 네이버페이 동기화 배송상품 재고 빼기
function subtract_naverpay_io_stock($it_id, $ct_qty, $io_id="", $io_type=0)
{
    global $g5;
...
// 프로모션 네이버 주문 클릭 로그
function promotion_naver_order_log($navercode)
{
    global $g5;

    $pa_user_code = get_cookie('ck_pa_user_code');
    $pa_code = get_cookie('ck_pa_code');

    if($pa_user_code) {
        $sql = " update {$g5['promotion_access_table']} set apa_od_naver = '$navercode' where apa_code = '$pa_user_code' ";
        sql_query($sql);

        $sql = " insert {$g5['promotion_naverpay_relation_table']} ( pa_code, apa_code, mall_code) values ( '{$pa_code}', '{$pa_user_code}', '{$navercode}' ) ";
        sql_query($sql);
    }
...
// 프로모션 네이버 주문 승인 로그
function promotion_naver_order($price, $od_id, $navercode='')
{
    global $g5;

    $sql = " select * from {$g5['promotion_access_table']} where apa_od_naver = '$navercode' ";
    $pno = sql_fetch($sql);

    if($pno) {
        $sql = " update {$g5['promotion_access_table']} set apa_purchase = '1', apa_purchase_val = '$price', apa_od_id = '$od_id' where apa_od_naver = '$navercode' ";
        sql_query($sql);

        $pa_code = $pno['pa_code'];
        $sql = " select pa_pur_val from {$g5['promotion_address_table']} where pa_code = '$pa_code' ";
        $row = sql_fetch($sql);

        $pa_pur_val = $row['pa_pur_val'] + (int)$price;
        
        $sql = " update {$g5['promotion_address_table']} set pa_purchase = pa_purchase + 1, pa_pur_val = '$pa_pur_val' where pa_code = '$pa_code' ";
        sql_query($sql);
    }
...
// 프로모션 주문 로그
function promotion_access_order($price, $od_id)
{
    global $g5;
    ...
    if($pa_user_code) {
        ...
        $sql = " update {$g5['promotion_access_table']} set apa_purchase = '1', apa_purchase_val = '$price', apa_od_id = '$od_id' where apa_code = '$pa_user_code' ";
        sql_query($sql);
...
// 프로모션 회원가입 로그
function promotion_access_register($mb_id)
{
    global $g5;
    ...
    if($pa_user_code) {
        $sql = " update {$g5['promotion_access_table']} set apa_reg_mb = '1', apa_mb_id = '$mb_id' where apa_code = '$pa_user_code' ";
        sql_query($sql);
    }
...
// 프로모션 로그인 계정연동
function promotion_access_login($mb_id)
{
    global $g5;

    $user_personal_code = get_cookie('user_personal_code');
    $pa_user_code = get_cookie('ck_pa_user_code');
    $chk_personal_code = get_cookie('ck_personal_code');
    $user_personal_device = get_cookie('ck_personal_device');

    if($user_personal_code && !$chk_personal_code) {
        $sql = " insert {$g5['promotion_member_relation_table']} ( pmr_mb_id, pmr_code, pmr_device) values ( '{$mb_id}', '{$user_personal_code}', '{$user_personal_device}' ) ";
        sql_query($sql);

        set_cookie("ck_personal_code", $user_personal_code, 60*60*24*3650);
    }

    if($pa_user_code) {
        $sql = " update {$g5['promotion_access_table']} set apa_mb_id = '$mb_id' where apa_code = '$pa_user_code' ";
        sql_query($sql);
    }
...
// 프로모션 행동 로그
function promotion_behavior_log($bhcode='')
{
    global $g5;
    ...
    if($pa_user_code) {
        $sql = " insert {$g5['promotion_behavior_table']} ( pa_code, apa_code, apa_user_code, bpa_date, bpa_time, bpa_url, bpa_url_code ) values ( '{$pa_code}', '{$pa_user_code}', '{$user_personal_code}', '".G5_TIME_YMD."', '".G5_TIME_HIS."', '', '{$bhcode}' ) ";
...
// 일반 행동 로그
function set_behavior_log($navercode='', $navertype='', $desc='', $od_id='', $bprice=0)
{
    global $g5;

    $sql = " insert {$g5['behavior_log_table']} ( bl_naver_code, bl_naver_type, bl_desc, bl_od_id, bl_price, bl_date, bl_time ) values ( '{$navercode}', '{$navertype}', '{$desc}', '{$od_id}', '{$bprice}', '".G5_TIME_YMD."', '".G5_TIME_HIS."' ) ";
    sql_query($sql);
