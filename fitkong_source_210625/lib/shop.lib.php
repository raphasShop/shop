...
function get_star_icon($is_star)
{
    if($is_star == 1) {
        $star = '<img src="'.G5_IMG_URL.'/rv_star_10.png"><img src="'.G5_IMG_URL.'/rv_star_0.png"><img src="'.G5_IMG_URL.'/rv_star_0.png"><img src="'.G5_IMG_URL.'/rv_star_0.png"><img src="'.G5_IMG_URL.'/rv_star_0.png">';
    } else if($is_star == 2) {
        $star = '<img src="'.G5_IMG_URL.'/rv_star_10.png"><img src="'.G5_IMG_URL.'/rv_star_10.png"><img src="'.G5_IMG_URL.'/rv_star_0.png"><img src="'.G5_IMG_URL.'/rv_star_0.png"><img src="'.G5_IMG_URL.'/rv_star_0.png">';
    } else if($is_star == 3) {
        $star = '<img src="'.G5_IMG_URL.'/rv_star_10.png"><img src="'.G5_IMG_URL.'/rv_star_10.png"><img src="'.G5_IMG_URL.'/rv_star_10.png"><img src="'.G5_IMG_URL.'/rv_star_0.png"><img src="'.G5_IMG_URL.'/rv_star_0.png">';
    } else if($is_star == 4) {
        $star = '<img src="'.G5_IMG_URL.'/rv_star_10.png"><img src="'.G5_IMG_URL.'/rv_star_10.png"><img src="'.G5_IMG_URL.'/rv_star_10.png"><img src="'.G5_IMG_URL.'/rv_star_10.png"><img src="'.G5_IMG_URL.'/rv_star_0.png">';
    } else if($is_star == 5) {
        $star = '<img src="'.G5_IMG_URL.'/rv_star_10.png"><img src="'.G5_IMG_URL.'/rv_star_10.png"><img src="'.G5_IMG_URL.'/rv_star_10.png"><img src="'.G5_IMG_URL.'/rv_star_10.png"><img src="'.G5_IMG_URL.'/rv_star_10.png">';
    }

    return $star;
}

function get_avg_star_icon($is_avg_star)
{
    $int_star = floor($is_avg_star);
    $star = get_star_icon($int_star);
    $is_star = $is_avg_star - $int_star;

    if($int_star == 1) {
        $star = '<img src="'.G5_IMG_URL.'/rv_star_10.png">';
    } else if($int_star == 2) {
        $star = '<img src="'.G5_IMG_URL.'/rv_star_10.png"><img src="'.G5_IMG_URL.'/rv_star_10.png">';
    } else if($int_star == 3) {
        $star = '<img src="'.G5_IMG_URL.'/rv_star_10.png"><img src="'.G5_IMG_URL.'/rv_star_10.png"><img src="'.G5_IMG_URL.'/rv_star_10.png">';
    } else if($int_star == 4) {
        $star = '<img src="'.G5_IMG_URL.'/rv_star_10.png"><img src="'.G5_IMG_URL.'/rv_star_10.png"><img src="'.G5_IMG_URL.'/rv_star_10.png"><img src="'.G5_IMG_URL.'/rv_star_10.png">';
    } else if($int_star == 5) {
        $star = '<img src="'.G5_IMG_URL.'/rv_star_10.png"><img src="'.G5_IMG_URL.'/rv_star_10.png"><img src="'.G5_IMG_URL.'/rv_star_10.png"><img src="'.G5_IMG_URL.'/rv_star_10.png"><img src="'.G5_IMG_URL.'/rv_star_10.png">';
    }
    //$is_star = is_double($is_star);

    //$is_star = string($is_star);

    if($is_star < 0.01 && $int_star == 5) {
        $star .= '';
    }else if($is_star < 0.01 && $int_star < 5) {
        $star .= '<img src="'.G5_IMG_URL.'/rv_star_0.png">';
    } else if($is_star < 0.11) {
        $star .= '<img src="'.G5_IMG_URL.'/rv_star_1.png">';
    } else if($is_star < 0.21) {
       $star .= '<img src="'.G5_IMG_URL.'/rv_star_2.png">';
    } else if($is_star < 0.31) {
       $star .= '<img src="'.G5_IMG_URL.'/rv_star_3.png">';
    } else if($is_star < 0.41) {
       $star .= '<img src="'.G5_IMG_URL.'/rv_star_4.png">';
    } else if($is_star < 0.51) {
       $star .= '<img src="'.G5_IMG_URL.'/rv_star_5.png">';
    } else if($is_star < 0.61) {
       $star .= '<img src="'.G5_IMG_URL.'/rv_star_6.png">';
    } else if($is_star < 0.71) {
       $star .= '<img src="'.G5_IMG_URL.'/rv_star_7.png">';
    } else if($is_star < 0.81) {
       $star .= '<img src="'.G5_IMG_URL.'/rv_star_8.png">';
    } else if($is_star < 0.91) {
       $star .= '<img src="'.G5_IMG_URL.'/rv_star_9.png">';
    } 

    
    if($int_star == 1) {
        $star .= '<img src="'.G5_IMG_URL.'/rv_star_0.png"><img src="'.G5_IMG_URL.'/rv_star_0.png"><img src="'.G5_IMG_URL.'/rv_star_0.png">';
    } else if($int_star == 2) {
        $star .= '<img src="'.G5_IMG_URL.'/rv_star_0.png"><img src="'.G5_IMG_URL.'/rv_star_0.png">';
    } else if($int_star == 3) {
        $star .= '<img src="'.G5_IMG_URL.'/rv_star_0.png">';
    } else if($int_star == 4) {
        $star .= '';
...
// 이모티콘 주소 전달
function get_review_emoticon($is_avg_star)
{
    if($is_avg_star < 1.5) {
        $star = 'rv_bad';
    } else if($is_avg_star < 2.5 && $is_avg_star >= 1.5) {
        $star = 'rv_sad';
    } else if($is_avg_star < 3.5 && $is_avg_star >= 2.5) {
        $star = 'rv_soso';
    } else if($is_avg_star < 4.5 && $is_avg_star >= 3.5) {
        $star = 'rv_good';
    } else if($is_avg_star >= 4.5) {
        $star = 'rv_best';
    } 
...
// 리뷰 평점 이모션 전달
function get_review_emotion($is_avg_star)
{
    if($is_avg_star < 1.5) {
        $star = '나빠요!';
    } else if($is_avg_star < 2.5 && $is_avg_star >= 1.5) {
...
// 상품이미지 썸네일 생성
function get_it_thumbnail($img, $width, $height=0, $i_width, $i_height, $id='', $is_crop=false)
{
    $str = '';
    ...
    if($thumb) {
        $file_url = str_replace(G5_PATH, G5_URL, $filepath.'/'.$thumb);
        $str = '<img src="'.$file_url.'" width="'.$i_width.'" height="'.$i_height.'"';
        if($id)
            $str .= ' id="'.$id.'"';
        $str .= ' alt="">';
    }
...
function get_option_stock_qty($it_id, $io_id, $type)
{
    ...
    $custom_io_id = '선택 :'.$io_id;
    $custom_io_id2 = '선택:'.$io_id;

    // 재고에서 빼지 않았고 주문인것만
    $sql = " select SUM(ct_qty) as sum_qty
               from {$g5['g5_shop_cart_table']}
              where it_id = '$it_id'
                and (ct_option = '{$custom_io_id}' || ct_option = '{$custom_io_id2}' || io_id = '{$io_id}')
                and io_type = '$type'
                and ct_stock_use = 0
                and ct_status in ('주문', '입금', '준비') ";
    $row = sql_fetch($sql);
...
{
    //comcose_edit_2018.04.23 회원정보 로드 시작
    global $member;
    //comcose_edit_2018.04.23 회원정보 로드 끝

    //comcose_edit_2018.04.23 로그인 쿼리 시작
    $sql_user_login = "select * from comcose_sellprice_level";
    $result_user_login = sql_query($sql_user_login);
    $user_login= sql_fetch_array($result_user_login);
    //comcose_edit_2018.04.23 로그인 쿼리 끝

    if ($tel_inq)
        $price = '전화문의';

    //comcose_edit_2018.04.23 로그인표시 시작
    else if ($user_login['cose_add_level_login'] == 1 and $member['mb_level'] == 1) $price = 'Login';
    //comcose_edit_2018.04.23 로그인표시 끝
...
function get_price($it)
{
    ...
    if (sql_num_rows($result_add_item) > 0) {
        ...
        //2등급
            ...
            for($i=0; $row_add_item = sql_fetch_array($result_add_item); $i++){
                if($it['it_id'] == $row_add_item['cose_item_add_it_id']){
                    if($row_add_item['cose_item_add_set_price_type2'] == '+' ){$price = $it['it_price'] + $row_add_item['cose_item_add_set_price2'];}
                    else if($row_add_item['cose_item_add_set_price_type2'] == '-' ){$price = $it['it_price'] - $row_add_item['cose_item_add_set_price2'];}
                    break 1;
                }
                //제외아이템
                else if (in_array($it['it_id'], $add_except_it_array)){$price = $it['it_price'];}
                ...
                //카테고리별 적용시
                    ...
                    for($i=0; $row_add_cat = sql_fetch_array($result_add_cat); $i++){
                        if($it['ca_id'] == $row_add_cat['cose_cat_add_cat_id'] or $it['ca_id2'] == $row_add_cat['cose_cat_add_cat_id'] or $it['ca_id3'] == $row_add_cat['cose_cat_add_cat_id']){
                            ...
                            //회원권한별
                            if ($add_user['cose_add_level_use'] == 1 ){
                                if($add_user['cose_add_level_2_type'] == '+' ){$price = $it['it_price'] + $level2;}else if($add_user['cose_add_level_2_type'] == '-' ){$price = $it['it_price'] - $level2;}
                            } else if ($add_user['cose_add_level_use'] == 0 ){$price = $it['it_price'];}
                        }
                    }
                }
        ...
        //3등급
            ...
            for($i=0; $row_add_item = sql_fetch_array($result_add_item); $i++){
                if($it['it_id'] == $row_add_item['cose_item_add_it_id']){
                    if($row_add_item['cose_item_add_set_price_type3'] == '+' ){$price = $it['it_price'] + $row_add_item['cose_item_add_set_price3'];}
                    else if($row_add_item['cose_item_add_set_price_type3'] == '-' ){$price = $it['it_price'] - $row_add_item['cose_item_add_set_price3'];}
                    break 1;
                }
                //제외아이템
                else if (in_array($it['it_id'], $add_except_it_array)){$price = $it['it_price'];}
                ...
                //카테고리별 적용시
                    ...
                    for($i=0; $row_add_cat = sql_fetch_array($result_add_cat); $i++){
                        if($it['ca_id'] == $row_add_cat['cose_cat_add_cat_id'] or $it['ca_id2'] == $row_add_cat['cose_cat_add_cat_id'] or $it['ca_id3'] == $row_add_cat['cose_cat_add_cat_id']){
                            ...
                            //회원권한별
                            if ($add_user['cose_add_level_use'] == 1 ){
                                if($add_user['cose_add_level_3_type'] == '+' ){$price = $it['it_price'] + $level3;}else if($add_user['cose_add_level_3_type'] == '-' ){$price = $it['it_price'] - $level3;}
                                else if ($member['mb_level'] == 10){$price = $it['it_price'];}
                            } else if ($add_user['cose_add_level_use'] == 0 ){$price = $it['it_price'];}
                        }
                    }
                }
        ...
        //4등급
            ...
            for($i=0; $row_add_item = sql_fetch_array($result_add_item); $i++){
                if($it['it_id'] == $row_add_item['cose_item_add_it_id']){
                    if($row_add_item['cose_item_add_set_price_type4'] == '+' ){$price = $it['it_price'] + $row_add_item['cose_item_add_set_price4'];}
                    else if($row_add_item['cose_item_add_set_price_type4'] == '-' ){$price = $it['it_price'] - $row_add_item['cose_item_add_set_price4'];}
                    break 1;
                }
                //제외아이템
                else if (in_array($it['it_id'], $add_except_it_array)){$price = $it['it_price'];}
                ...
                //카테고리별 적용시
                    ...
                    for($i=0; $row_add_cat = sql_fetch_array($result_add_cat); $i++){
                        if($it['ca_id'] == $row_add_cat['cose_cat_add_cat_id'] or $it['ca_id2'] == $row_add_cat['cose_cat_add_cat_id'] or $it['ca_id3'] == $row_add_cat['cose_cat_add_cat_id']){
                            ...
                            //회원권한별
                            if ($add_user['cose_add_level_use'] == 1 ){
                                if($add_user['cose_add_level_4_type'] == '+' ){$price = $it['it_price'] + $level4;}else if($add_user['cose_add_level_4_type'] == '-' ){$price = $it['it_price'] - $level4;}
                            } else if ($add_user['cose_add_level_use'] == 0 ){$price = $it['it_price'];}
                        }
                    }
                }
        ...
        //5등급
            ...
            for($i=0; $row_add_item = sql_fetch_array($result_add_item); $i++){
                if($it['it_id'] == $row_add_item['cose_item_add_it_id']){
                    if($row_add_item['cose_item_add_set_price_type5'] == '+' ){$price = $it['it_price'] + $row_add_item['cose_item_add_set_price5'];}
                    else if($row_add_item['cose_item_add_set_price_type5'] == '-' ){$price = $it['it_price'] - $row_add_item['cose_item_add_set_price5'];}
                    break 1;
                }
                //제외아이템
                else if (in_array($it['it_id'], $add_except_it_array)){$price = $it['it_price'];}
                ...
                //카테고리별 적용시
                    ...
                    for($i=0; $row_add_cat = sql_fetch_array($result_add_cat); $i++){
                        if($it['ca_id'] == $row_add_cat['cose_cat_add_cat_id'] or $it['ca_id2'] == $row_add_cat['cose_cat_add_cat_id'] or $it['ca_id3'] == $row_add_cat['cose_cat_add_cat_id']){
                            ...
                            //회원권한별
                            if ($add_user['cose_add_level_use'] == 1 ){
                                if($add_user['cose_add_level_5_type'] == '+' ){$price = $it['it_price'] + $level5;}else if($add_user['cose_add_level_5_type'] == '-' ){$price = $it['it_price'] - $level5;}
                            } else if ($add_user['cose_add_level_use'] == 0 ){$price = $it['it_price'];}
                        }
                    }
                }
        ...
        //6등급
            ...
            for($i=0; $row_add_item = sql_fetch_array($result_add_item); $i++){
                if($it['it_id'] == $row_add_item['cose_item_add_it_id']){
                    if($row_add_item['cose_item_add_set_price_type6'] == '+' ){$price = $it['it_price'] + $row_add_item['cose_item_add_set_price6'];}
                    else if($row_add_item['cose_item_add_set_price_type6'] == '-' ){$price = $it['it_price'] - $row_add_item['cose_item_add_set_price6'];}
                    break 1;
                }
                //제외아이템
                else if (in_array($it['it_id'], $add_except_it_array)){$price = $it['it_price'];}
                ...
                //카테고리별 적용시
                    ...
                    for($i=0; $row_add_cat = sql_fetch_array($result_add_cat); $i++){
                        if($it['ca_id'] == $row_add_cat['cose_cat_add_cat_id'] or $it['ca_id2'] == $row_add_cat['cose_cat_add_cat_id'] or $it['ca_id3'] == $row_add_cat['cose_cat_add_cat_id']){
                            ...
                            //회원권한별
                            if ($add_user['cose_add_level_use'] == 1 ){
                                if($add_user['cose_add_level_6_type'] == '+' ){$price = $it['it_price'] + $level6;}else if($add_user['cose_add_level_6_type'] == '-' ){$price = $it['it_price'] - $level6;}
                            } else if ($add_user['cose_add_level_use'] == 0 ){$price = $it['it_price'];}
                        }
                    }
                }
        ...
        //7등급
            ...
            for($i=0; $row_add_item = sql_fetch_array($result_add_item); $i++){
                if($it['it_id'] == $row_add_item['cose_item_add_it_id']){
                    if($row_add_item['cose_item_add_set_price_type7'] == '+' ){$price = $it['it_price'] + $row_add_item['cose_item_add_set_price7'];}
                    else if($row_add_item['cose_item_add_set_price_type7'] == '-' ){$price = $it['it_price'] - $row_add_item['cose_item_add_set_price7'];}
                    break 1;
                }
                //제외아이템
                else if (in_array($it['it_id'], $add_except_it_array)){$price = $it['it_price'];}
                ...
                //카테고리별 적용시
                    ...
                    for($i=0; $row_add_cat = sql_fetch_array($result_add_cat); $i++){
                        if($it['ca_id'] == $row_add_cat['cose_cat_add_cat_id'] or $it['ca_id2'] == $row_add_cat['cose_cat_add_cat_id'] or $it['ca_id3'] == $row_add_cat['cose_cat_add_cat_id']){
                            ...
                            //회원권한별
                            if ($add_user['cose_add_level_use'] == 1 ){
                                if($add_user['cose_add_level_7_type'] == '+' ){$price = $it['it_price'] + $level7;}else if($add_user['cose_add_level_7_type'] == '-' ){$price = $it['it_price'] - $level7;}
                            } else if ($add_user['cose_add_level_use'] == 0 ){$price = $it['it_price'];}
                        }
                    }
                }
        ...
        //8등급
            ...
            for($i=0; $row_add_item = sql_fetch_array($result_add_item); $i++){
                if($it['it_id'] == $row_add_item['cose_item_add_it_id']){
                    if($row_add_item['cose_item_add_set_price_type8'] == '+' ){$price = $it['it_price'] + $row_add_item['cose_item_add_set_price8'];}
                    else if($row_add_item['cose_item_add_set_price_type8'] == '-' ){$price = $it['it_price'] - $row_add_item['cose_item_add_set_price8'];}
                    break 1;
                }
                //제외아이템
                else if (in_array($it['it_id'], $add_except_it_array)){$price = $it['it_price'];}
                ...
                //카테고리별 적용시
                    ...
                    for($i=0; $row_add_cat = sql_fetch_array($result_add_cat); $i++){
                        if($it['ca_id'] == $row_add_cat['cose_cat_add_cat_id'] or $it['ca_id2'] == $row_add_cat['cose_cat_add_cat_id'] or $it['ca_id3'] == $row_add_cat['cose_cat_add_cat_id']){
                            ...
                            //회원권한별
                            if ($add_user['cose_add_level_use'] == 1 ){
                                if($add_user['cose_add_level_8_type'] == '+' ){$price = $it['it_price'] + $level8;}else if($add_user['cose_add_level_8_type'] == '-' ){$price = $it['it_price'] - $level8;}
                            } else if ($add_user['cose_add_level_use'] == 0 ){$price = $it['it_price'];}
                        }
                    }
                }
        ...
        //9등급
            ...
            for($i=0; $row_add_item = sql_fetch_array($result_add_item); $i++){
                if($it['it_id'] == $row_add_item['cose_item_add_it_id']){
                    if($row_add_item['cose_item_add_set_price_type9'] == '+' ){$price = $it['it_price'] + $row_add_item['cose_item_add_set_price9'];}
                    else if($row_add_item['cose_item_add_set_price_type9'] == '-' ){$price = $it['it_price'] - $row_add_item['cose_item_add_set_price9'];}
                    break 1;
                }
                //제외아이템
                else if (in_array($it['it_id'], $add_except_it_array)){$price = $it['it_price'];}
                ...
                //카테고리별 적용시
                    ...
                    for($i=0; $row_add_cat = sql_fetch_array($result_add_cat); $i++){
                        if($it['ca_id'] == $row_add_cat['cose_cat_add_cat_id'] or $it['ca_id2'] == $row_add_cat['cose_cat_add_cat_id'] or $it['ca_id3'] == $row_add_cat['cose_cat_add_cat_id']){
                            ...
                            //회원권한별
                            if ($add_user['cose_add_level_use'] == 1 ){
                                if($add_user['cose_add_level_9_type'] == '+' ){$price = $it['it_price'] + $level9;}else if($add_user['cose_add_level_9_type'] == '-' ){$price = $it['it_price'] - $level9;}
                            } else if ($add_user['cose_add_level_use'] == 0 ){$price = $it['it_price'];}
                        }
                    }
                }
    ...
    //아이템별 미적용시
        ...
        //제외아이템
        if (in_array($it['it_id'], $add_except_it_array)){$price = $it['it_price'];}
        //제외카테고리
        else if (in_array($it['ca_id'], $add_except_cat_array) or in_array($it['ca_id2'], $add_except_cat_array) or in_array($it['ca_id3'], $add_except_cat_array)){$price = $it['it_price'];}
        //카테고리별 적용시
        else if (sql_num_rows($result_add_cat) > 0) {
            for($i=0; $row_add_cat = sql_fetch_array($result_add_cat); $i++){
                if($it['ca_id'] == $row_add_cat['cose_cat_add_cat_id'] or $it['ca_id2'] == $row_add_cat['cose_cat_add_cat_id'] or $it['ca_id3'] == $row_add_cat['cose_cat_add_cat_id']){
                    ...
                    //회원권한별
                    if ($add_user['cose_add_level_use'] == 1 ){
                        if ($member['mb_level'] == 1){if($add_user['cose_add_level_1_type'] == '+' or $add_user['cose_add_level_1_type'] == ''){$price = $it['it_price'] + $level1;}else if($add_user['cose_add_level_1_type'] == '-' ){$price = $it['it_price'] - $level1;}}
                        else if ($member['mb_level'] == 2){if($add_user['cose_add_level_2_type'] == '+' ){$price = $it['it_price'] + $level2;}else if($add_user['cose_add_level_2_type'] == '-' ){$price = $it['it_price'] - $level2;}}
                        else if ($member['mb_level'] == 3){if($add_user['cose_add_level_3_type'] == '+' ){$price = $it['it_price'] + $level3;}else if($add_user['cose_add_level_3_type'] == '-' ){$price = $it['it_price'] - $level3;}}
                        else if ($member['mb_level'] == 4){if($add_user['cose_add_level_4_type'] == '+' ){$price = $it['it_price'] + $level4;}else if($add_user['cose_add_level_4_type'] == '-' ){$price = $it['it_price'] - $level4;}}
                        else if ($member['mb_level'] == 5){if($add_user['cose_add_level_5_type'] == '+' ){$price = $it['it_price'] + $level5;}else if($add_user['cose_add_level_5_type'] == '-' ){$price = $it['it_price'] - $level5;}}
                        else if ($member['mb_level'] == 6){if($add_user['cose_add_level_6_type'] == '+' ){$price = $it['it_price'] + $level6;}else if($add_user['cose_add_level_6_type'] == '-' ){$price = $it['it_price'] - $level6;}}
                        else if ($member['mb_level'] == 7){if($add_user['cose_add_level_7_type'] == '+' ){$price = $it['it_price'] + $level7;}else if($add_user['cose_add_level_7_type'] == '-' ){$price = $it['it_price'] - $level7;}}
                        else if ($member['mb_level'] == 8){if($add_user['cose_add_level_8_type'] == '+' ){$price = $it['it_price'] + $level8;}else if($add_user['cose_add_level_8_type'] == '-' ){$price = $it['it_price'] - $level8;}}
                        else if ($member['mb_level'] == 9){if($add_user['cose_add_level_9_type'] == '+' ){$price = $it['it_price'] + $level9;}else if($add_user['cose_add_level_9_type'] == '-' ){$price = $it['it_price'] - $level9;}}
                        else if ($member['mb_level'] == 10){$price = $it['it_price'];}
                    } else if ($add_user['cose_add_level_use'] == 0 ){$price = $it['it_price'];}
                }
            }
    ...
    //comcose_edit_2018.04.23 금액조정 끝
    //$price = $it['it_price']; comcose_edit_금액조정 표시로 주석처리
...
function get_cust_price($it)
{
    global $member;

    if ($it['it_tel_inq']) return '';

    $price = $it['it_cust_price'];
...
{
    return number_format($point, 0).' 원';
}
...
// 변경후 상품선택옵션 가격표시 수정 (가격이 변함없을때 +0원표시 없앰) 아이스크림 열기 <!-- 시작
    ...
    if($subj_count > 1) {
        ...
        for($i=0; $row=sql_fetch_array($result); $i++) {
           ...
           if($_SERVER['REMOTE_ADDR']=="121.172.7.103") {
                 $select .= '<option value="'.$row['io_id'].','.$row['io_price'].','.$row['io_stock_qty'].'">'.$row['io_id'].$price.$soldout.$io_stock_qty.'</option>'.PHP_EOL;
             } else {
...
function item_icon($it)
{
    ...
    if ($it['it_type1'])
        $icon .= '<img src="'.G5_IMG_URL.'/acm-home-best-badge-online.png" class="icon_img">';

    if ($it['it_type2'])
        $icon .= '<img src="'.G5_IMG_URL.'/acm-home-best-badge-mdpick.png" class="icon_img">';

    if ($it['it_type3'])
        $icon .= '<img src="'.G5_IMG_URL.'/acm-home-best-badge-new.png" class="icon_img">';

    if ($it['it_type4'])
        $icon .= '<img src="'.G5_IMG_URL.'/acm-home-best-badge-best.png" class="icon_img">';

    if ($it['it_type5'])
        $icon .= '<img src="'.G5_IMG_URL.'/acm-home-best-badge-sale.png" class="icon_img">';
...
function get_coupon_count($mb_id)
{
...
// 변경후 : 상품유형 및 쿠폰정보를 상품목록/상세페이지에 표현하기 - 아이스크림 열기 <!-- 시작

#####################################################################################

// (1) 상품유형(type) 아이콘 대신 타이틀이름으로 출력 2016-4-26
function item_typetext($it)
{
    global $g5;
	
	$sql = " select de_type1_title, de_type2_title, de_type3_title, de_type4_title, de_type5_title from {$g5['g5_shop_default_table']}";
    $row = sql_fetch($sql);
	
	$ittype1 = $row['de_type1_title'];
	$ittype2 = $row['de_type2_title'];
	$ittype3 = $row['de_type3_title'];
	$ittype4 = $row['de_type4_title'];
	$ittype5 = $row['de_type5_title'];
	
	$typetext = '<span class="gray font-11">';
    // 품절
    if (is_soldout($it['it_id']))
        $typetext .= '<img src="'.G5_SHOP_URL.'/img/icon_soldout.gif" alt="품절">';
    // 상품유형
    if ($it['it_type1'])
        $typetext .= '<span class="orange font-11"> '.$ittype1.' </span>';

    if ($it['it_type2'])
        $typetext .= '<span class="green font-11"> '.$ittype2.' </span>';

    if ($it['it_type3'])
        $typetext .= '<span class="blue font-11"> '.$ittype3.' </span>';

    if ($it['it_type4'])
        $typetext .= '<span class="crimson font-11"> '.$ittype4.' </span>';

    if ($it['it_type5'])
        $typetext .= '<span class="skyblue font-11"> '.$ittype5.' </span>';

    $typetext .= '</span>';

    return $typetext;
}
// (2) 개별상품/카테고리 할인쿠폰 목록에 표시 출력 2016-4-26
function item_coupon($it)
{
    global $g5;
...
// (3) 배송비할인쿠폰 목록에 표시 출력 2016-4-26
function item_deliverycoupon($it)
{
    global $g5;
...
// (4) 주문할인쿠폰 목록에 표시 출력 2016-4-26
function item_ordercoupon($it)
{
    global $g5;
...
// (5) 품절시 블라인드(상품이미지위에 커튼효과) 2016-4-26
function item_soldout($it)
{
    global $g5;
	
	$soldout = '<span class="lightgray font-11">';
    // 품절
    if (is_soldout($it['it_id']))
        $soldout .= '<div class="label-soldout">SOLD OUT<br><span class="font-11">이상품은 판매완료되었습니다</span></div>';
...
// 변경후 (장바구니 상태추가에 따른 수정) - 아이스크림 <!-- 시작

#####################################################################################

// 주문의 금액, 배송비 과세금액 등의 정보를 가져옴 (아이스크림 소스수정)
function get_order_info($od_id)
{
    ...
    $sql = " select SUM(IF(io_type = 1, (io_price * ct_qty), ((ct_price + io_price) * ct_qty))) as price,
                ...
                from {$g5['g5_shop_cart_table']}
                where od_id = '$od_id'
                  and ct_status IN ( '주문', '입금', '준비', '배송', '완료', '교환' ) ";
    $sum = sql_fetch($sql);
...
function get_item_point($it, $io_id='', $trunc=10)
{
    ...
    if($it['it_point_type'] > 0) {
        /* comcose_edit_2017.11.09 위젯마스터님 오류 발견 및 도움으로 수정함
         * 회원등급의 상품별 금액설정된 금액에 대한 포인트가 아닌 원가격에 대한 포인트가 적립되는 오류
         * 위젯 마스터님 제안 $it_price = $it['it_price']; 을 $it_price = get_price($it); 로변경
         */
...
// 상품후기 주소 조작 방지
function check_itemuse_modulation($od_id, $mb_id)
{
    global $g5, $default, $is_admin;
...
// <타입1> 배송조회버튼 생성(주문내역 내용 페이지에 노출) 아이스크림 소스수정
...
// <타입2> 배송조회버튼 생성(관리자 주문내역 목록에 노출/송장번호에링크) 아이스크림 소스수정
function get_delivery_list_inquiry($company, $invoice, $class='')
{
    if(!$company || !$invoice)
        return '';
    ...
    if($com && $url) {
        ...
        if($class)
            $str .= ' class="'.$class.'"';
        $str .='>'.$invoice.'&nbsp;<i class="fa fa-plus-square" aria-hidden="true"></i></a>'; //2016-07-20추가.배송번호표시아이콘
        if($tel)
            $str .= '<br>(문의:'.$tel.')';
    }
...
function get_wishlist_count_by_item($it_id='')
{
    ...
    $sql = "select count(a.it_id) as num from {$g5['g5_shop_wish_table']} a where a.it_id  = $it_id";
    $row = sql_fetch($sql);
