...
$row = sql_fetch($sql);
if ($w == '' && $row['mb_id']) {
    alert("입력하신 본인확인 정보와 동일한 카카오 계정 가입내역이 존재합니다.",G5_URL);
}
...
if ($w == '') {
    ...
    if ($config['cf_use_recommend'] && $mb_recommend) {
        ...
        if($w == '') {
            ...
            if($create_coupon) {
                $cp_subject = '추천인 주문할인 쿠폰';
                $cp_method = 2;
                $cp_target = '';
                $cp_start = G5_TIME_YMD;
                $cp_end = date("Y-m-d", (G5_SERVER_TIME + (86400 * 10 * 365)));
                $cp_type = 0;
                $cp_price = 2000;
                $cp_trunc = 1;
                $cp_minimum = 0;
                ...
                $sql = " INSERT INTO {$g5['g5_shop_coupon_table']}
                        ...
                        VALUES
                            ( '$cp_id', '$cp_subject', '$cp_method', '$cp_target', '$mb_recommend', '$cp_start', '$cp_end', '$cp_type', '$cp_price', '$cp_trunc', '$cp_minimum', '$cp_maximum', '".G5_TIME_YMDHIS."' ) ";
    ...
    promotion_access_register($mb_id);
    // promotion address : behavior register
    promotion_behavior_log('register');
