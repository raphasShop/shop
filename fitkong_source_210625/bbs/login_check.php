...
// 로그인 시 쿠폰 증정
$login_coupon_use = true;


$sql_lc = " select count(*) as cnt from {$g5['g5_shop_coupon_table']} where cp_subject = '핏콩에 쿠폰꽃이 피나봄' and mb_id = '$mb_id' ";
$row_lc = sql_fetch($sql_lc);

if($row_lc['cnt']) {
    $login_coupon_use = false;
}

$startdate_str = "2021-04-07 00:00:00";
$enddate_str = "2021-04-15 00:00:00";
...
if($startdiff >= 0 && $enddiff < 0) {echo $login_coupon_use;} else {
    $login_coupon_use = false;
}
...
// promotion address : access register
promotion_access_login($mb_id);
// promotion address : behavior register
promotion_behavior_log('login'); 
