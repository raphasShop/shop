...
class Hybrid_Providers_Kakao extends Hybrid_Provider_Model_OAuth2
{
    ...
    */
    function getUserProfile()
    {
        ...
        $params = array('property_keys'=>array('kakao_account.email','kakao_account.birthday','kakao_account.gender','kakao_account.age_range,kakao_account.ci'));        // v2 parameter

        $params2 = array('property_keys'=>array('kakao_account.email','kakao_account.birthday','kakao_account.gender','kakao_account.age_range'));        // v2 parameter
        ...
        $delivery_data = $this->api->api("user/shipping_address", "GET","");

        $this->api->api_base_url  = "https://kapi.kakao.com/v1/";
        $channel_data = $this->api->api("api/talk/plusfriends", "GET","");

        $this->api->api_base_url  = "https://kapi.kakao.com/v1/";
        $terms_data = $this->api->api("user/service/terms", "GET","");
        ...
        $birthday = @ $data->kakao_account->birthday;   // v2 version

        if( $birthday ){
            $this->user->profile->birthday = $birthday;
        }

        $birthyear = @ $data->kakao_account->birthyear;   // v2 version

        if( $birthyear ){
            $this->user->profile->birthyear = $birthyear;
        }


        $gender = @ $data->kakao_account->gender;   // v2 version

        if( $gender ){
            $this->user->profile->gender = $gender;
        }

        $age_range = @ $data->kakao_account->age_range;   // v2 version

        if( $age_range ){
            $this->user->profile->age_range = $age_range;
        }

        $phone_number = @ $data->kakao_account->phone_number;   // v2 version

        if( $phone_number ){
            $this->user->profile->phone_number = $phone_number;
        }

        $plusfriends = @ $channel_data->plus_friends[0]->relation;   // v2 version

        if( $plusfriends ){
            $this->user->profile->plusfriends = $plusfriends;
        }

        $account_ci = @ $data->kakao_account->ci_authenticated_at;   // v2 version

        if( $account_ci ){
            $this->user->profile->account_ci = $account_ci;
        }

        $this->user->profile->data = $data;
        $this->user->profile->terms = $terms_data;


        $shipping_addresses = @ $delivery_data->shipping_addresses;   // v2 version
        if( $shipping_addresses ){
            $this->user->profile->shipping_addresses = $shipping_addresses;
        }
