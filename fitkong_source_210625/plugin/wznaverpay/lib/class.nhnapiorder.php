    ...
    function orderSync($status = '') {
        ...
        foreach ($arr_data as $k => $v) {
            ...
            if ($tmp_OrderID <> $OrderID) {
                $od_id = get_uniqid(); // 2019-09-25 : wetoz
            }
            $tmp_OrderID = $OrderID;
            ...
            foreach ($arr_cart as $k2 => $v2) {
                ...
                $ct = sql_fetch("select ct_id, ct_qty, ct_status, io_id, io_type from {$g5['g5_shop_cart_table']} where ProductOrderID = '".$ProductOrderID."' ");
                if ($ct['ct_id']) {
                    $query = " update {$g5['g5_shop_cart_table']} set ct_status = '".$v2['ct_status']."' where ct_id = '".$ct['ct_id']."' ";
                    sql_query($query, true);

                    // 네이버페이 주문 동기화 재고 처리
                    if($v2['ct_status'] == '배송' && $ct['ct_status'] != $v2['ct_status']) {
                        if($ct['io_id']) {
                            subtract_naverpay_io_stock($ct['it_id'], $ct['ct_qty'], $ct['io_id'], $ct['io_type']);
                        } else {
                            subtract_naverpay_io_stock($ct['it_id'], $ct['ct_qty']);
                        }
                    }

                    if($v2['ct_status'] == '완료' && $ct['ct_status'] != $v2['ct_status'] && $ct['ct_status'] != '배송') {
                        if($ct['io_id']) {
                            subtract_naverpay_io_stock($ct['it_id'], $ct['ct_qty'], $ct['io_id'], $ct['io_type']);
                        } else {
                            subtract_naverpay_io_stock($ct['it_id'], $ct['ct_qty']);
                    ...
                    $query = "select ct_id, ct_qty, ct_status, io_id, io_type, mb_id from {$g5['g5_shop_cart_naverpay_table']} where MallManageCode = '".$MallManageCode."'";
                    ...
                    if ($MallManageCode && $ct['ct_id']) {
                        ...
                        // 네이버페이 주문 동기화 재고 처리
                        if($v2['ct_status'] == '배송' || $v2['ct_status'] == '완료') {
                            if($ct['io_id']) {
                                subtract_naverpay_io_stock($ct['it_id'], $ct['ct_qty'], $ct['io_id'], $ct['io_type']);
                            } else {
                                subtract_naverpay_io_stock($ct['it_id'], $ct['ct_qty']);
                        ...
                        // 네이버페이 주문 동기화 재고 처리
                        $product_option = addslashes($v2['ProductOption']);
                        $filter_name = ':';
                        if(strpos($product_option,$filter_name) !== false) {
                            $io_id_array = explode($filter_name, $product_option);
                            $io_id = $io_id_array[1];
                        }
                        if($v2['ct_status'] == '배송' || $v2['ct_status'] == '완료') {
                            if($io_id) {
                                subtract_naverpay_io_stock($it_id, $v2['ct_qty'], $io_id);
                            } else {
                                subtract_naverpay_io_stock($it_id, $v2['ct_qty']);
                            }
                        }
