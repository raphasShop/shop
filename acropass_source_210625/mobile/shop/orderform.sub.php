...
?>
    ...
    <ul class="sod_list">
        ...
        for ($i=0; $row=sql_fetch_array($result); $i++)
        {
            ...
            if($is_member) {
                ...
                $sql = " select it_4 from {$g5['g5_shop_item_table']} where it_id = '{$row['it_id']}'";
                $it_no_cp = sql_fetch($sql);

                if($it_no_cp['it_4'] == '1') {
                    $coupon_unused = 1;
                    $coupon_unused_item = 1;
                }
            }
    ...
    <div class="sod_ta_wr">
        <dl id="m_sod_bsk_tot">
            ...
            <dt class="sod_bsk_point">임직원 할인</dt>
            <dd class="sod_bsk_point"><strong>-<?php echo number_format($member_discount); ?> 원</strong></dd>
            <?php } ?>
            -->
    ...
    <section id="sod_frm_pay">
        ...
        <div class="odf_tbl">
            ...
            <tr>
                ...
                <td colspan="2"><input type="text" name="coupon_code" class="frm_input" id="coupon_code" value="" placeholder="쿠폰코드입력" style="font-size:13px; padding:6px 20px;width:85vw;max-width:85vw;text-align: center;"></td>
            ...
            <tr>
                <td colspan="2" style="text-align: center;background: #fafafa">
                    <?php if($coupon_unused_item == '1') { ?>
                        특별 이벤트 상품이 포함되어 있어<br>쿠폰 및 적립금을 중복 사용하실 수 없습니다
                    <?php } else { ?>
        ...
        if ($is_member && $config['cf_use_point'])
        {
            ...
            if ($member['mb_point'] >= $default['de_settle_min_point'])
            {
                ...
                if($coupon_unused == 0) {
                    ...
                    echo '<div><input type="hidden" name="max_temp_point" value="'.$temp_point.'"><label for="od_temp_point">사용 포인트('.$point_unit.'점 단위)</label> <input type="text" id="od_temp_point" name="od_temp_point" value="0" class="frm_input" size="10" onKeyup="point_change(this.value);">  점</div>';
...
gtag('event', 'add_to_cart', {
  "items": [
    <?php for($g=0; $g<$i; $g++) { ?>{
      "id": "<?php echo $item[$g]['it_id']; ?>",
      "name": "<?php echo $item[$g]['it_name']; ?>",
    }<?php if($g+1<$i) echo ','; ?>
    <?php } ?>
  ]
});
