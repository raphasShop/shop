...
<?php if($ref) { ?>
    <div id="top_sub_gnb">
        <div class="top_sub_wrap"><?php echo $g5['title']; ?></div>
...
<div id="sod_fin">
    ...
    ?>        
        <div id="sod_fin_no"><strong>주문이 정상적으로 완료되었습니다. 아래 가상계좌로 입금을 요청드립니다.</strong> <br> <?php echo $od['od_bank_account']; ?></div>
        <?php if($cp_offer == 1) { ?>
    ...
    ?>
        <div id="sod_fin_no"><strong>결제가 정상적으로 완료되었습니다.</strong> <br> 상품이 준비중입니다. 발송 후 알림톡으로 안내드리겠습니다.</div>
        <?php if($cp_offer == 1) { ?>
...
if (!_nasa) var _nasa={};
_nasa["cnv"] = wcs.cnv("1","<?php echo $tot_price; ?>"); // 구매완료 전환 스크립트
wcs_do(_nasa);
</script> 
<script>
gtag('event', 'purchase', {
  "transaction_id": "<?php echo $od_id; ?>",
  "affiliation": "Acropass Shop",
  "value": <?php echo $tot_price; ?>,
  "currency": "KRW",
  "tax": 0,
  "shipping": <?php echo $send_cost; ?>,
  "items": [
    <?php for($g=0; $g<$i; $g++) { ?>{
      "id": "<?php echo $item[$g]['it_id']; ?>",
      "name": "<?php echo $item[$g]['it_name']; ?>",
      "quantity": <?php echo $item[$g]['it_qty']; ?>,
      "price": "<?php echo $item[$g]['it_price']; ?>"
    }<?php if($g+1<$i) echo ','; ?>
    <?php } ?>
  ]
});
