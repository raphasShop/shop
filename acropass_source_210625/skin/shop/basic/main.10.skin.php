...
for ($i=0; $row=sql_fetch_array($result); $i++) {
    ...
    if ($this->view_it_img) {
        echo get_it_image($row['it_id'], 400, 400, '', '', stripslashes($row['it_name']))."\n";               
    }
    ...
    if ($row['it_10']) {
        echo "<div class=\"event_badge_msg\" style=\"background:".$row['it_9'].";color:".$row['it_8'].";\">".$row['it_10']."</div>\n";
    }
    ...
    if ($this->view_it_price) {
        if(get_cust_price($row) != get_price($row) && get_cust_price($row) != 0) {
        echo "<div class=\"sct_cust_cost\">\n";
        echo display_price(get_cust_price($row), $row['it_tel_inq'])."\n";
        echo "</div>\n";
        }
        echo "<div class=\"sct_cost\">\n";
        echo display_price(get_price($row), $row['it_tel_inq'])."\n";
        echo "</div>\n";
        echo "<div class=\"sct_favorite\">\n";
        echo "<span><i class=\"xi-heart-o\"></i> &nbsp;".get_wishlist_count_by_item($row['it_id'])."</span>";
        echo "<span>리뷰 &nbsp;".$row['it_use_cnt']."</span>";
        echo "</div>\n";
    }
