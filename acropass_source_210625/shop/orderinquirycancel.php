...
// 변경후 (주문취소 배송비 가격변경 수정) 아이스크림 열기 <!--

#####################################################################################

// 주문 취소 (배송비 0원입력 제거) (아이스크림 소스수정)
$cancel_memo = addslashes(strip_tags($cancel_memo));
$cancel_price = ($od['od_cart_price'] + $od['od_send_cost'] + $od['od_send_cost2']) - ($od['od_cart_coupon'] + $od['od_send_coupon'] + $od['od_coupon']); //취소금액(상품가격+배송비-쿠폰)
...
// --> 닫기
// 추가로 교환,환불,품절,반품 신청시 상태 변경되는것을 추가할 수 있음
...
// 주문취소 회원의 쿠폰을 되돌려 줌 
if ($od['od_coupon'] > 0 || $od['od_send_coupon'] > 0) 
