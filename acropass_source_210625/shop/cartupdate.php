    ...
    for($i=0; $i<$count; $i++) {
        ...
        for($k=0; $k<$opt_count; $k++) {
            ...
            if($config['cf_use_point']) {
                ...
                //$point_new  = $point + (($member['mb_level']-2)*2*($point/3));
            ...
            */
            ...
            $sql .= $comma."( '$tmp_cart_id', '{$member['mb_id']}', '{$it['it_id']}', '".addslashes($it['it_name'])."', '{$it['it_sc_type']}', '{$it['it_sc_method']}', '{$it['it_sc_price']}', '{$it['it_sc_minimum']}', '{$it['it_sc_qty']}', '쇼핑', '$cart_price', '$point', '0', '0', '$io_value', '$ct_qty', '{$it['it_notax']}', '$io_id', '$io_type', '$io_price', '".G5_TIME_YMDHIS."', '$REMOTE_ADDR', '$ct_send_cost', '$sw_direct', '$ct_select', '$ct_select_time' )";
