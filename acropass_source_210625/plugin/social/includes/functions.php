...
function social_return_from_provider_page( $provider, $login_action_url, $mb_id, $mb_password, $url, $use_popup=2 ){
    ...
    if( !G5_SOCIAL_USE_POPUP || strpos($ref, 'login_check.php') !== false ){
        if( get_session('social_login_redirect') ){
            ...
            //goto_url(G5_BBS_URL.'/register.php?provider='.urlencode($url));
    ...
    if( $provider_name ){
        ...
        //소셜 데이터와 회원데이터가 일치 하는 경우 계정와 연결할지, 새로 계정을 만들지 선택합니다.
            ...
            */
            ...
            // 소셜로그인 리다이렉트 수정 
            //$register_url = G5_BBS_URL.'/register.php?provider='.$provider_name;
            if( $url ){
            ...
            ?>
                <script>
                    if( window.opener )
                    {
                        ...
                        if (typeof window.social_link_fn != 'undefined')
                        {
                            window.social_link_fn("<?php echo $provider_name; ?>");
                        } else {
            ...
            <?php
                ...
                echo "<script>console.log('use popup');";
