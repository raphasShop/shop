...
class NHNAPIORDER extends NHNAPISCL {
    ...
    function orderSync($status = '') {
        ...
        $xml = $this->GetChangedProductOrderList();
        ...
        foreach ($req->ChangedProductOrderInfoList as $k => $v) {
            ...
            if (strtoupper((string)$v->LastChangedStatus) === 'PAY_WAITING') // 입금대기는 제외
                continue;
        ...
        foreach ($arr_data as $k => $v) {
            ...
            $od_name = $od_tel = $od_hp = $od_delivery_company = $od_invoice = '';
            $od_mobile = 0;
            ...
            foreach ($req->ProductOrderInfoList as $k2 => $v2) {
                ...
                if (!$od_name) {
                    ...
                    $ZipCode    = (string)$v2->ProductOrder->ShippingAddress->ZipCode;
                ...
                //$od_cart_price += ($row['ct_price'] * $row['ct_qty']); // 2019-06-03
            ...
            sql_query("LOCK TABLE {$g5['g5_shop_cart_table']} READ, LOCK TABLE {$g5['g5_shop_cart_table']} WRITE ", false);

            foreach ($arr_cart as $k2 => $v2) {

                $ProductOrderID = trim($v2['ProductOrderID']);

                $ct = sql_fetch("select ct_id from {$g5['g5_shop_cart_table']} where ProductOrderID = '".$ProductOrderID."' ");
                if ($ct['ct_id']) {
                    ...
                    $ct = sql_fetch("select ct_id, mb_id from {$g5['g5_shop_cart_naverpay_table']} where MallManageCode = '".$MallManageCode."' and ct_select = 0");
                    if ($MallManageCode && $ct['ct_id']) {

                        $mb_id = $ct['mb_id'];

                        // 장바구니 등록 : 이렇게되면 ProductOrderID 등록불가? ProductOrderID 가 등록이 안되면 취소처리 불가
                        $query = "insert into {$g5['g5_shop_cart_table']}(od_id, mb_id, it_id, it_name, it_sc_type, it_sc_method, it_sc_price, it_sc_minimum, it_sc_qty, ct_price, ct_point, ct_point_use, ct_stock_use, ct_option, ct_qty, ct_notax, io_id, io_type, io_price, ct_time, ct_ip, ct_send_cost, ct_select, ct_select_time, ct_status, ProductOrderID) select '".$od_id."', mb_id, it_id, it_name, it_sc_type, it_sc_method, it_sc_price, it_sc_minimum, it_sc_qty, ct_price, ct_point, ct_point_use, ct_stock_use, ct_option, ct_qty, ct_notax, io_id, io_type, io_price, ct_time, ct_ip, ct_send_cost, 1, '".G5_TIME_YMDHIS."', '".$v2['ct_status']."', '".$ProductOrderID."' from {$g5['g5_shop_cart_naverpay_table']} where MallManageCode = '".$MallManageCode."' and ct_select = 0 ";
                        sql_query($query, true);
                      
                        // 적용완료처리.
                        sql_query(" delete from {$g5['g5_shop_cart_naverpay_table']} where MallManageCode = '".$MallManageCode."' and ct_select = 0 ");


                        //promotion address : naverpay order sync
                        //$s_cart_id = $ct['s_cart_id'];
                        //promotion_naver_cart($od_cart_price, $MallManageCode);


                        // promotion address : naverpay order sync
                        promotion_naver_order($od_cart_price,$od_id,$MallManageCode);
                        set_behavior_log($MallManageCode,'Managecode','',$od_id,$od_cart_price);
                        //promotion_behavior_log('naverpayorder');
                    }
                    else {
                        ...
                        $it_name_c = $v2['it_name'];
                        $it_name_c = str_replace('\'','',$it_name_c);

                        $it_option_c = $v2['ProductOption'];
                        $it_option_c = str_replace('\'','',$it_option_c);
                        ...
                        $query = "insert into {$g5['g5_shop_cart_table']} set
                                ...
                                ct_option = '".$it_option_c."',
                                ct_select = '1',
                                ProductOrderID = '".$ProductOrderID."'
                        ";
                        sql_query($query, true);
                        set_behavior_log($MallManageCode,'Nocode','',$od_id,$od_cart_price);
                    }
                }
            }
            ...
            $query = "select od_id from {$g5['g5_shop_order_table']} where od_id = '".$od_id."'";
            $od = sql_fetch($query);
    ...
    function inquirysync() {
        ...
        foreach ($arr_iq as $k => $v) {
            ...
            if ($iq['iq_id']) { // 존재하므로 업데이트
                ...
                $query = "update {$g5['g5_shop_item_qa_table']} set
                            iq_subject = '".$v['iq_subject']."',
                            iq_question = '".$v['iq_question']."',
                            iq_answercontentid = '".$v['iq_answercontentid']."',
                            iq_answer = '".$v['iq_answer']."'
                where iq_id = '".$iq['iq_id']."'";
                sql_query($query);
            }
            else {
                ...
                $query = "insert into {$g5['g5_shop_item_qa_table']} set
                            it_id = '".$it_id."',
                            mb_id = '".$v['mb_id']."',
                            iq_name = '".$v['iq_name']."',
                            iq_subject = '".$v['iq_subject']."',
                            iq_question = '".$v['iq_question']."',
                            iq_time = '".$v['iq_time']."',
                            iq_inquiryid = '".$v['iq_inquiryid']."',
                            iq_productorderid = '".$v['iq_productorderid']."',
                            iq_orderid = '".$v['iq_orderid']."',
                            iq_answercontentid = '".$v['iq_answercontentid']."',
                            iq_answer = '".$v['iq_answer']."'
                        ";
                sql_query($query, true);
    ...
    // 상품 주문 변경 내역 조회
    function GetChangedProductOrderList($status = '') {
