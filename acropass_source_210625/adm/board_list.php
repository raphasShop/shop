...
<!-- 검색창 -->
<div class="dan-schbox1" style="text-align:center;margin-bottom:10px;"><!-- 검색창 -->
...
<div class="local_desc03 local_desc">
    <div class="totallist"><strong><?php echo number_format($total_count) ?></strong> 개의 게시판이 검색되었습니다</div>
    ...
    <select id="ssch_sort" onChange="window.location.href=this.value">
        ...
        <option value="<?php echo $sortlist; ?>bo_subject&amp;sod=asc">게시판 제목순</option>
        <option value="<?php echo $sortlist; ?>a.gr_id&amp;sod=asc">게시판 그룹명순</option>
        <option value="<?php echo $sortlist; ?>bo_table&amp;sod=asc">테이블명 순</option>
        <option value="<?php echo $sortlist; ?>bo_order&amp;sod=asc">출력순서 순</option>
        <option value="<?php echo $sortlist; ?>bo_use_search&amp;sod=desc">검색 사용순</option>
        <option value="<?php echo $sortlist; ?>bo_use_sns&amp;sod=desc">SNS 사용순</option>
        <option value="<?php echo $sortlist; ?>bo_device&amp;sod=asc">접속기기 순</option>
        <option value="<?php echo $sortlist; ?>a.gr_id, a.bo_table&amp;sod=asc">정렬초기화</option>
    </select>
    </section>
    <!-- } 정렬 선택 끝 -->
    </div>
...
<div id="bq_right1">
    ...
    <?php if ($is_admin == 'super') { // 게시판추가버튼 ?>
    <div class="bq_basic_add">
    <a href="./board_form.php" id="bo_add"><i class="fa fa-plus"></i> 게시판추가</a>
    </div>
    <?php } ?>
</div>
<!--//-->
