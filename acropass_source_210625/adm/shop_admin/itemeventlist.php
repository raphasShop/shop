<?php
$sub_menu = '422310'; /* 수정전 원본 메뉴코드 500310 */
...
?>
<!-- 선택창 -->
<div class="dan-schbox1" style="text-align:center; border:solid 1px #3CB4F5;background:#64C3F5;"><!-- 이벤트선택창 -->
    <div class="row"><!-- row 시작 { -->
<form name="flist" class="big_sch01 big_sch" autocomplete="off">
...
<select name="ev_id" id="ev_id" action="<?php echo $_SERVER['SCRIPT_NAME']; ?>">
    ...
    while ($row1=sql_fetch_array($result1))
        $event_option .= '<option value="'.$row1['ev_id'].'" '.get_selected($ev_id, $row1['ev_id']).' >'.conv_subject($row1['ev_subject'], 40,"…").'</option>';
    echo $event_option;
    ?>
</select>
<input type="submit" value="이동" class="btn_submit_big">
...
<div class="local_desc01 local_desc">
    <p align="center">상품을 이벤트별로 일괄 처리합니다. <?php echo ($ev_title ? '현재 선택된 이벤트는 "'.$ev_title.'" 입니다.' : '이벤트를 선택해 주세요.'); ?></p>
</div>
<!-- // -->

<!-- 검색창 -->
<div class="dan-schbox-none" style="text-align:center;"><!-- 검색창 -->
    <div class="row"><!-- row 시작 { -->
<form name="flist" class="big_sch01 big_sch" autocomplete="off">
<input type="hidden" name="page" value="<?php echo $page; ?>">
...
<input type="text" name="search" value="<?php echo $search; ?>" id="search" required class="frm_input_big required">
...
<div class="local_desc03 local_desc">
    <strong><?php echo number_format($total_count) ?></strong> 개의 상품이 검색되었습니다.
</div>
<!-- // -->
