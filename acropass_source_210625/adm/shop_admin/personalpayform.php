...
<div id="bq_right1">   
    ...
    <?php if($popup == 'yes') { ?>
        <button type="button" onclick="self.close();">닫기</button>
    <?php } else { ?>
        <a href="./personalpaylist.php?<?php echo $qstr; ?>"><i class="fas fa-tasks"></i> 목록</a>
    <?php } ?>
    </div>

    <?php if($w == 'u') { ?>
    <div class="bq_basic">
    <a href="./personalpayformupdate.php?w=d&amp;pp_id=<?php echo $pp['pp_id']; ?>" onclick="return delete_confirm(this);"><i class="fas fa-cut"></i> 삭제</a>
    </div>
    <?php } ?>
