...
<button type="button" onClick="location.href='<?php echo G5_ADMIN_URL; ?>/shop_admin/itemselloptionrank.php'"><i class="fa fa-refresh" aria-hidden="true"></i> 선택초기화</button>
</div>
</form>
<!-- 검색창 끝 -->
...
<div class="local_desc03 local_desc">
    <strong><?php echo number_format($total_count); ?></strong> 개의 상품순위별 옵션구매건수가 검색되었습니다
</div>
<!-- // -->
...
<div class="tbl_head01 tbl_wrap">
    ...
    <tr>
        <th scope="col">순위</th>
        <th scope="col" colspan="2">상품명<br>옵션 (배송완료상품수)</th>
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++)
    {
        ...
        <tr class="<?php echo $bg; ?>">
            ...
            <td><a href="<?php echo $href; ?>"><?php echo cut_str($row['it_name'],30); ?></a>
            <?php
            if($row['ct_status_6'] > 0) {
                $res  = sql_query(" select ct_option, SUM(ct_qty) as qty from {$g5['g5_shop_cart_table']} where it_id = '{$row['it_id']}' and ct_status = '완료' group by ct_option ");
                for ($j=0; $opt=sql_fetch_array($res); $j++)
                    echo '<br><img src="'.G5_ADMIN_URL.'/img/left-plus.gif"> <span class="violet">'.trim($opt['ct_option']).' : '.$opt['qty'].' 개 배송완료</span>';
            } else {
                echo '';
            }
            ?>
