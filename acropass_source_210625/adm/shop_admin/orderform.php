...
//환불금액 자동입력 체크
$html_refund_chk = '<input type="checkbox" id="od_refund_chk" value="'.$od['od_receipt_price'].'" onclick="chk_refund_price()">
<label for="od_refund_chk">결제취소/환불금액 입력</label><br>';
...
<section id="anc_sodr_list">
    <?php echo $pg_anchor; ?>
    <h2 class="h2_frm">주문상품 목록&nbsp;&nbsp;<?php echo $od['od_id']; ?></h2>
    ...
    <div class="tbl_head01 tbl_wrap">
        ...
        for($i=0; $row=sql_fetch_array($result); $i++) {
            ...
            <tr>
                ...
                <td>
                    <?php echo get_text($opt['ct_option']); //옵션황목명?>
                </td>
                ...
                <td class="td_ox"><?php echo ($opt['ct_point_use']) ? '<font style="font-size:14px;color:#6DA5D8;">●</font>' : '×'; //포인트반영?></td>
                <td class="td_ox"><?php echo ($opt['ct_stock_use']) ? '<font style="font-size:14px;color:#6DA5D8;">●</font>' : '×'; //재고반영?></td>
            </tr>
            <?php
                $chk_cnt++;
            }
    ...
    <div class="btn_list02 btn_list">
        <p>
            ...
            <input type="submit" name="ct_status" value="주문" onclick="document.pressed=this.value" class="at-tip" data-original-title="<nobr>입금대기</nobr>" data-toggle="tooltip" data-placement="bottom" data-html="true">
            <input type="submit" name="ct_status" value="입금" onclick="document.pressed=this.value" class="at-tip" data-original-title="<nobr>결제완료</nobr>" data-toggle="tooltip" data-placement="bottom" data-html="true">
            <input type="submit" name="ct_status" value="준비" onclick="document.pressed=this.value" class="at-tip" data-original-title="<nobr>배송준비중</nobr>" data-toggle="tooltip" data-placement="bottom" data-html="true">
            <input type="submit" name="ct_status" value="배송" onclick="document.pressed=this.value" class="at-tip" data-original-title="<nobr>택배발송</nobr>" data-toggle="tooltip" data-placement="bottom" data-html="true">
            <input type="submit" name="ct_status" value="완료" onclick="document.pressed=this.value" class="at-tip" data-original-title="<nobr>배송완료</nobr>" data-toggle="tooltip" data-placement="bottom" data-html="true">
            <input type="submit" name="ct_status" value="교환" onclick="document.pressed=this.value" class="at-tip" data-original-title="<nobr>상품하자,고객변심으로 인한<br>상품교환</nobr>" data-toggle="tooltip" data-placement="bottom" data-html="true">
            <input type="submit" name="ct_status" value="취소" onclick="document.pressed=this.value" class="at-tip" data-original-title="<nobr>입금전취소</nobr>" data-toggle="tooltip" data-placement="bottom" data-html="true">
            <input type="submit" name="ct_status" value="품절" onclick="document.pressed=this.value" class="at-tip" data-original-title="<nobr>재고가없어<br>주문취소 및 환불</nobr>" data-toggle="tooltip" data-placement="bottom" data-html="true">
            <input type="submit" name="ct_status" value="환불" onclick="document.pressed=this.value" class="at-tip" data-original-title="<nobr>결제(입금)완료후<br>상품배송전 취소로 인한 환불</nobr>" data-toggle="tooltip" data-placement="bottom" data-html="true">
            <input type="submit" name="ct_status" value="반품" onclick="document.pressed=this.value" class="at-tip" data-original-title="<nobr>고객변심으로<br>반품 및 환불</nobr>" data-toggle="tooltip" data-placement="bottom" data-html="true">
        </p>
    </div>

    <div class="local_desc01 local_desc">
        <div style="display:inline-block;vertical-align:top;">
        <b>주문 및 장바구니 상태변경 버튼 안내</b><br>
        <span class="infobtn">취소</span> 입금전취소&nbsp;&nbsp;
        <span class="infobtn">품절</span> 재고가 없어 주문취소&nbsp;&nbsp;
        <span class="infobtn">환불</span> 결제완료후 상품배송전 취소로 인한 환불<br>
        <span class="infobtn">교환</span> 상품하자로 인한 상품교환&nbsp;&nbsp;
        <span class="infobtn">반품</span> 고객변심으로 인한 반품환불<br><br>
        
        주문, 입금, 준비, 배송, 완료는 장바구니와 주문서 상태를 모두 변경하지만, 취소, 반품, 품절은 장바구니의 상태만 변경하며, 주문서 상태는 변경하지 않습니다.<br>
        개별적인(이곳에서의) 상태 변경은 모든 작업을 수동으로 처리합니다.
        <br>예를 들어 주문에서 입금으로 상태 변경시 입금액(결제금액)을 포함한 모든 정보는 수동 입력으로 처리하셔야 합니다.<br><br>
        <b>[포인트적립]</b> 구매완료 후 지정기일후에 적립<br>
        <b>[재고반영]</b> 배송시 재고에서 차감됨<br>
        </div>
    </div>
    ...
    <div class="compare_wrap">
        ...
        <section id="anc_sodr_chk" class="compare_left">
            ...
            <div class="tbl_frm01">
                ...
                <tr>
                    <th scope="row" class="lightviolet">결제취소/환불액</th>
                    <td>
                    <?php echo ($od['od_refund_price'] > 0) ? '<span class="violet">'.display_price($od['od_refund_price']).'</span>' : '-';?>
                    </td>
                </tr>
        ...
        <section id="anc_sodr_paymo" class="compare_right">
            ...
            <div class="tbl_frm01">
                ...
                <?php if ($od['od_settle_case'] == '무통장' || $od['od_settle_case'] == '가상계좌') { ?>
                <tr>
                    <td scope="row" colspan="2"><label for="od_bank_account"><b>계좌번호</b></label></td>
                </tr>
                <tr>
                    <td colspan="2"><?php echo $bank_account; ?></td>
                </tr>
                <?php } ?>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <input type="text" name="od_receipt_time" value="<?php echo is_null_time($od['od_receipt_time']) ? "" : $od['od_receipt_time']; ?>" id="op_receipt_time" class="frm_input w100per" maxlength="19">
                    </td>
                </tr>
                <?php } ?>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <input type="text" name="od_receipt_time" value="<?php echo is_null_time($od['od_receipt_time']) ? "" : $od['od_receipt_time']; ?>" id="od_receipt_time" class="frm_input w100per" maxlength="19">
                    </td>
                </tr>
                <?php } ?>
                ...
                <tr>
                    <th scope="row"><label for="od_refund_price" class="lightviolet">결제취소/환불 금액</label></th>
                    <td>
                        <?php echo $html_refund_chk; //자동입력 체크?>
...
<section id="anc_sodr_memo">
    ...
    <div class="local_desc02 local_desc">
        <p>
            현재 열람중인 주문에 대해 관리자가 일처리를 위해 작성하는 전용 메모공간입니다. <strong>관리자만 볼수 있으며,주문자에게는 보여지지 않습니다.</strong><br>
            [자동기록] 입금,배송내역을 메일로 발송할 경우 함께 기록됩니다.<br>
            [자동기록] 고객이 취소처리한것도 함께 기록됩니다.
        </p>
    </div>
...
<section>
    ...
    <div class="compare_wrap">
        ...
        <section id="anc_sodr_orderer" class="compare_left">
            <h3><i class="far fa-user fa-lg gray"></i> 주문하신 분</h3>
            ...
            <div class="tbl_frm01">
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <input type="text" name="od_addr1" value="<?php echo get_text($od['od_addr1']); ?>" id="od_addr1" required class="frm_input w100per required">
                        <label for="od_addr1">기본주소</label><br>
                        <input type="text" name="od_addr2" value="<?php echo get_text($od['od_addr2']); ?>" id="od_addr2" class="frm_input w100per">
                        <label for="od_addr2">상세주소</label>
                        <br>
                        <input type="text" name="od_addr3" value="<?php echo get_text($od['od_addr3']); ?>" id="od_addr3" class="frm_input w100per">
                ...
                <tr>
                    ...
                    <td><input type="text" name="od_email" value="<?php echo $od['od_email']; ?>" id="od_email" required class="frm_input email required w100per"></td>
                </tr>
        ...
        <section id="anc_sodr_taker" class="compare_right" style="border:solid 1px #A8DDFA;">
            <h3><i class="fas fa-user-plus fa-lg skyblue"></i> 받으시는 분</h3>
            ...
            <div class="tbl_frm01">
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <input type="text" name="od_b_addr1" value="<?php echo get_text($od['od_b_addr1']); ?>" id="od_b_addr1" required class="frm_input required w100per">
                        <label for="od_b_addr1">기본주소</label>
                        <input type="text" name="od_b_addr2" value="<?php echo get_text($od['od_b_addr2']); ?>" id="od_b_addr2" class="frm_input w100per">
                        <label for="od_b_addr2">상세주소</label>
                        <input type="text" name="od_b_addr3" value="<?php echo get_text($od['od_b_addr3']); ?>" id="od_b_addr3" class="frm_input w100per">
...
function form_submit(f)
{
    ...
    if(status == "취소" || status == "반품" || status == "품절" || status == "환불") {
    ...
    if (confirm(msg+"\'" + status + "\' 상태를 선택하셨습니다.\n\n선택하신대로 처리하시겠습니까?\n\n무통장입금건은 환불처리후 결제취소/환불금액을 직접 기재해주세요")) {
        return true;
    } else {
        return false;
    }
...
// 취소,환불금액 수동 설정 - 아이스크림 기능 추가
function chk_refund_price()
{
    var chk = document.getElementById("od_refund_chk");
    var price = document.getElementById("od_refund_price");
    price.value = chk.checked ? (parseInt(chk.value) + parseInt(price.defaultValue)) : price.defaultValue;
}
</script>
