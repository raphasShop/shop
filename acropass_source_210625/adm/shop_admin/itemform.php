<?php
$sub_menu = '400333'; /* 수정전 원본 메뉴코드 400300 */
...
if ($w == "" && $wadd !== "yes" || $w == "")
{
    ...
    // 신규등록시에는 옵션의 쿠키에 저장된 값을 보여주지않음 - 크림장수
	// 다른상품 마지막 저장한 카테고리가 자동으로 표시되는데, 신규등록하는 입장에서는 굉장히 헷갈림
	// 연속등록하는 경우에만 앞서 등록한 카테고리가 표시되게 함
...
else if ($w == "" && $wadd == "yes")
{
    ...
    // 옵션은 쿠키에 저장된 값을 보여줌. 다음 입력을 위한것임 - 크림장수(기본분류만표시)
    //$it[ca_id] = _COOKIE[ck_ca_id];
    $it['ca_id'] = get_cookie("ck_ca_id");
    $it['ca_id2'] = get_cookie("ck_ca_id2");
    $it['ca_id3'] = get_cookie("ck_ca_id3");
...
<li><a href="#anc_sitfrm_compact">상품정보제공고시</a></li>
...
$frm_submit = '<div class="btn_confirm01 btn_confirm"><div class="h10"></div>';
/*if($it_id)
...
<?php if ($w == 'u') { //수정일경우?>
<div class="div_userscore">
    <!-- (1) 상품등록 수정일 -->
    <span class="gray">최종수정일 : </span><span class="darkgray"><?php echo $it['it_update_time']; //최종수정일?></span>
    <span class="gray">(<?php echo substr($it['it_time'],0,10); //등록일?> 등록)</span>
    <!--(1)//-->
    <!-- (2) 후기/문의/판매갯수 -->
    <div class="pull-right">
        <?php if ($score = get_star_image($it['it_id'])) { //사용후기가 있을경우?>
        <!-- 고객후기평점 -->
        <span class="sound_only"> 고객선호도별<?php echo $score?>개</span>
        <span class="pink">고객후기평점</span>
        ...
        <span class="go_link"><a href="<?php echo G5_ADMIN_URL;?>/shop_admin/itemuselist.php?sfl=a.it_id&stx=<?php echo $it['it_id']; ?>" target="_blank">후기보기</a></span>
        <!--//-->
        <?php } ?>
        <!-- 상품문의 -->
        &nbsp;&nbsp;<span class="go_link2"><a href="<?php echo G5_ADMIN_URL;?>/shop_admin/itemqalist.php?sfl=a.it_id&stx=<?php echo $it['it_id']; ?>" target="_blank">상품문의보기</a></span>
        <!--//-->
        <!-- 판매갯수 -->
        &nbsp;&nbsp;<span class="darkgray">상품판매갯수</span>
        <span class="screen_tack1"><?php echo ($sales_qty > '0') ? '<b>'.number_format($sales_qty).'</b><span class="gray">개</span>' : '0<span class="gray">개</span>';//판매상품갯수?></span>
        <!--//-->
    </div>
    <!--(2)//-->
</div>
...
<section id="anc_sitfrm_cate">
    ...
    <div class="local_desc02 local_desc">
        <p>
        총3개의 카테고리에 상품을 등록할 수 있습니다. 기본 분류는 반드시 선택하셔야 합니다.<br>
        하나의 상품에 최대 3개의 다른 분류에 등록할 수 있습니다. 상품을 등록하고자하는 맨하위 카테고리를 선택하면, 자동으로 상위카테고리에도 표시됩니다.<br>
        <b>[예제] </b>
        선풍기를 <strong>계절가전>선풍기>가정용선풍기</strong> 와 <strong>여름특가상품>선풍기</strong> 에 모두 등록하고 싶다면<br>
        <b>계절가전>선풍기>가정용선풍기</b> 를 기본분류로 선택하고, <b>여름특가상품>선풍기</b> 를 추가분류로 등록하면 2군데 카테고리에서 모두 노출됩니다.
        </p>
    </div>
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        <tbody>
        <tr>
            <th scope="row"><label for="ca_id"><span class="orangered">[필수]</span> 기본분류등록 </label></th>
            <td>
                ...
                {  
                    ...
                    for ($i=1; $i<=$len; $i++)
                    {
                        ...
                        $sql_caid = " select ca_name from {$g5['g5_shop_category_table']} where ca_id = '$code' ";
                        $caid = sql_fetch($sql_caid);
	                	// 1차카테고리에는 ＞표시않함 시작
                        ...
                        // 1차카테고리에는 ＞표시않함 끝
                        $navigation .= '<span class="gray">'.$bar.'<a href="'.G5_SHOP_URL.'/list.php?ca_id='.$code.'" target="_blank"> <span class="violet" style="letter-spacing:-1px;">'.$caid['ca_name'].'</span></a>';
                    }
                }
        ...
        <!-- ca_id2 추가카테고리 -->
        <tr>
            <th scope="row"><label for="ca_id2"><span class="gray font-normal">[선택]</span>  추가분류등록</label></th>
            <td>
                <?php echo help(' 카테고리 추가등록 : 기본 분류외에 다른 분류에도 등록하고 싶을때, 맨하위 분류를 선택하시면 해당분류에도 상품이 등록됩니다.'); ?>
                <select name="ca_id2" id="ca_id2">
                    <option value="">선택하세요</option>
                    <?php echo conv_selected_option($category_select, $it['ca_id2']); ?>
                </select>
                <?php
                // ca_id2 추가분류 카테고리 위치표시 시작
                echo '&nbsp;';
                ...
                {  
                    ...
                    for ($i=1; $i<=$len; $i++)
                    {
                        $code2 = substr($ca_id2,0,$i*2);

                        $sql_caid2 = " select ca_name from {$g5['g5_shop_category_table']} where ca_id = '$code2' ";
                        $caid2 = sql_fetch($sql_caid2);
		                // 1차카테고리에는 ＞표시않함 시작
		                if($code2 == substr($code2, 0, 2)) {
	                        $bar = "";
	                    } else {
	                        $bar = " ＞";
	                    }
						// 1차카테고리에는 ＞표시않함 끝
                        $navigation2 .= '<span class="gray">'.$bar.'<a href="'.G5_SHOP_URL.'/list.php?ca_id='.$code2.'" target="_blank"> <span class="violet" style="letter-spacing:-1px;">'.$caid2['ca_name'].'</span></a>';
                    }
                }
        ...
        <!-- ca_id3 추가카테고리 -->
        <tr>
            <th scope="row"><label for="ca_id3"><span class="gray font-normal">[선택]</span> 추가분류등록</label></th>
            <td>
                <?php echo help(' 카테고리 추가등록 : 기본 분류외에 다른 분류에도 등록하고 싶을때, 맨하위 분류를 선택하시면 해당분류에도 상품이 등록됩니다.'); ?>
                ...
                {  
                    ...
                    for ($i=1; $i<=$len; $i++)
                    {
                        $code3 = substr($ca_id3,0,$i*2);

                        $sql_caid3 = " select ca_name from {$g5['g5_shop_category_table']} where ca_id = '$code3' ";
                        $caid3 = sql_fetch($sql_caid3);
		                // 1차카테고리에는 ＞표시않함 시작
		                if($code3 == substr($code3, 0, 2)) {
	                        $bar = "";
	                    } else {
	                        $bar = " ＞";
	                    }
						// 1차카테고리에는 ＞표시않함 끝
                        $navigation3 .= '<span class="gray">'.$bar.'<a href="'.G5_SHOP_URL.'/list.php?ca_id='.$code3.'" target="_blank"> <span class="violet" style="letter-spacing:-1px;">'.$caid3['ca_name'].'</span></a>';
                    }
                }
        ...
        <?php for ($i=2; $i<=3; $i++) { ?>
        <tr>
            <th scope="row"><label for="ca_id<?php echo $i; ?>"><?php echo $i; ?>. 추가분류등록</label></th>
            <td>
                <?php echo help(' 카테고리 추가등록 : 기본 분류외에 다른 분류에도 등록하고 싶을때, 맨하위 분류를 선택하시면 해당분류에도 상품이 등록됩니다.'); ?>
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        <tr>
            ...
            <td colspan="2">
                <?php if ($w == '') { // 추가 ?>
                    ...
                    <input type="text" name="it_id" value="<?php echo time(); ?>" id="it_id" required class="frm_input required" maxlength="20">
        ...
        <tr>
            ...
            <td colspan="2">
                ...
                <input type="text" name="it_name" value="<?php echo get_text(cut_str($it['it_name'], 250, "")); ?>" id="it_name" required class="frm_input required w100per">
            </td>
        </tr>
        <tr>
            ...
            <td>
                ...
                <input type="text" name="it_basic" value="<?php echo get_text($it['it_basic']); ?>" id="it_basic" class="frm_input w100per">
            </td>
            <td class="td_grpset">
        ...
        <tr>
            <th scope="row"><label for="it_10">이벤트메시지</label></th>
            <td colspan="2">
                <?php echo help("메인 및 상품 리스트 페이지와 상품 상세페이지에 이벤트 메시지가 출력됩니다"); ?>
                <input type="text" name="it_10" value="<?php echo get_text(cut_str($it['it_10'], 250, "")); ?>" id="it_10" class="frm_input w100per">
            </td>
        </tr>
        <tr>
            <th scope="row"><label for="it_9">이벤트메시지 배경컬러</label></th>
            <td colspan="2">
                <?php echo help("이벤트 메시지의 배경색을 지정합니다."); ?>
                <input id="it_9" class="frm_input iColorPicker" type="text" name="it_9" value="<?php echo get_text($it['it_9']); ?>">
            </td>
        </tr>
        <tr>
            <th scope="row"><label for="it_8">이벤트메시지 폰트컬러</label></th>
            <td colspan="2">
                <?php echo help("이벤트 메시지의 폰트색을 지정합니다."); ?>
                <input id="it_8" class="frm_input iColorPicker" type="text" name="it_8" value="<?php echo get_text($it['it_8']); ?>">
            </td>
        </tr>
        <tr>
            ...
            <td>
                ...
                <input type="text" name="it_order" value="<?php echo $it['it_order']; ?>" id="it_order" class="frm_input" size="10">
            </td>
            <td class="td_grpset">
        ...
        <tr>
            <th scope="row">온라인상품출력순서</th>
            <td>
                 <?php echo help("숫자가 작을 수록 상위에 출력됩니다."); ?>
                <input type="text" name="it_2" value="<?php echo get_text($it['it_2']) ?>" id="it_2" class="frm_input">
            </td>
            <td class="td_grpset">
                <input type="checkbox" name="chk_ca_2" value="1" id="chk_ca_1>">
                <label for="chk_ca_2">분류적용</label>
                <input type="checkbox" name="chk_all_2" value="1" id="chk_all_1">
        ...
        <tr>
            <th scope="row"><label for="it_11">상품유형</label></th>
            <td>
                <?php echo help("상품이 속해있는 유형을 선택합니다."); ?>
                <select name="it_11" id="it_11">
                    <option value="needle"<?php echo get_selected('needle', $it['it_11']); ?>>마이크로니들</option>
                    <option value="skincare"<?php echo get_selected('skincare', $it['it_11']); ?>>스킨케어</option>
                </select>
            </td>
            <td class="td_grpset">
                <input type="checkbox" name="chk_ca_it_11" value="1" id="chk_ca_it_11">
                <label for="chk_ca_it_11">분류적용</label>
                <input type="checkbox" name="chk_all_it_11" value="1" id="chk_all_it_11">
                <label for="chk_all_it_11">전체적용</label>
            </td>
        </tr>
        <tr>
            <th scope="row"><label for="it_12">상품목적</label></th>
            <td>
                <?php echo help("상품이 속해있는 유형을 선택합니다."); ?>
                <select name="it_12" id="it_12">
                    <option value="trouble"<?php echo get_selected('trouble', $it['it_12']); ?>>트러블/진정</option>
                    <option value="whitening"<?php echo get_selected('whitening', $it['it_12']); ?>>색소침착/미백</option>
                    <option value="antiaging"<?php echo get_selected('antiaging', $it['it_12']); ?>>안티에이징</option>
                </select>
            </td>
            <td class="td_grpset">
                <input type="checkbox" name="chk_ca_it_12" value="1" id="chk_ca_it_12">
                <label for="chk_ca_it_12">분류적용</label>
                <input type="checkbox" name="chk_all_it_12" value="1" id="chk_all_it_12">
                <label for="chk_all_it_12">전체적용</label>
            </td>
        </tr>
        <tr>
            ...
            <td>
                ...
                <input type="text" name="it_maker" value="<?php echo get_text($it['it_maker']); ?>" id="it_maker" class="frm_input">
            </td>
            <td class="td_grpset">
        ...
        <tr>
            ...
            <td>
                <?php echo help("입력하지 않으면 상품상세페이지에 출력하지 않습니다."); ?>
                <input type="text" name="it_origin" value="<?php echo get_text($it['it_origin']); ?>" id="it_origin" class="frm_input">
            </td>
            <td class="td_grpset">
        ...
        <tr>
            ...
            <td>
                <?php echo help("입력하지 않으면 상품상세페이지에 출력하지 않습니다."); ?>
                <input type="text" name="it_brand" value="<?php echo get_text($it['it_brand']); ?>" id="it_brand" class="frm_input">
            </td>
            <td class="td_grpset">
        ...
        <tr>
            ...
            <td>
                <?php echo help("입력하지 않으면 상품상세페이지에 출력하지 않습니다."); ?>
                <input type="text" name="it_model" value="<?php echo get_text($it['it_model']); ?>" id="it_model" class="frm_input">
            </td>
            <td class="td_grpset">
        ...
        <tr>
            ...
            <td>
                ...
                <label for="it_nocoupon"><span class="blue font-14 font-bold">on</span> 으로 설정시 <b>쿠폰이 적용되지 않습니다</b></label>
            </td>
            <td class="td_grpset">
        ...
        <tr>
            <th scope="row"><label for="it_4">주문시쿠폰적용안함</label></th>
            <td>
                <?php echo help("설정에 체크하시면 상품 주문 시 쿠폰이 적용되지 않습니다."); ?>
                <label class="switch-check">
                <input type="checkbox" name="it_4" value="1" id="it_4" <?php echo ($it['it_4']) ? "checked" : ""; ?>> 예
                <div class="check-slider round"></div>
                </label>
                <label for="it_4"><span class="blue font-14 font-bold">on</span> 으로 설정시 <b>쿠폰이 적용되지 않습니다</b></label>
            </td>
            <td class="td_grpset">
                <input type="checkbox" name="chk_ca_it_4" value="1" id="chk_ca_it_4">
                <label for="chk_ca_it_4">분류적용</label>
                <input type="checkbox" name="chk_all_it_4" value="1" id="chk_all_it_4">
                <label for="chk_all_it_4">전체적용</label>
            </td>
        </tr>
        <tr>
            <th scope="row"><label for="it_5">상품비공개</label></th>
            <td>
                <?php echo help("설정에 체크하시면 상품을 외부로 노출하지 않습니다."); ?>
                <label class="switch-check">
                <input type="checkbox" name="it_5" value="1" id="it_5" <?php echo ($it['it_5']) ? "checked" : ""; ?>> 예
                <div class="check-slider round"></div>
                </label>
                <label for="it_5"><span class="blue font-14 font-bold">on</span> 으로 설정시 <b>상품이 자사몰에서 노출되지 않습니다.</b></label>
            </td>
            <td class="td_grpset">
                <input type="checkbox" name="chk_ca_it_5" value="1" id="chk_ca_it_5">
                <label for="chk_ca_it_5">분류적용</label>
                <input type="checkbox" name="chk_all_it_5" value="1" id="chk_all_it_5">
                <label for="chk_all_it_5">전체적용</label>
            </td>
        </tr>
        <tr>
            <th scope="row"><label for="it_7">회원만구매가능</label></th>
            <td>
                <?php echo help("자사몰에 가입한 회원들만 구매가 가능하도록 제공하는 옵션입니다."); ?>
                <select name="it_7" id="it_7">
                    <option value=""<?php echo get_selected('', $it['it_7']); ?>>모든접속자구매가능</option>
                    <option value="member"<?php echo get_selected('member', $it['it_7']); ?>>회원만구매가능</option>
                </select>
            </td>
            <td class="td_grpset">
                <input type="checkbox" name="chk_ca_it_7" value="1" id="chk_ca_it_7">
                <label for="chk_ca_it_7">분류적용</label>
                <input type="checkbox" name="chk_all_it_7" value="1" id="chk_all_it_7">
                <label for="chk_all_it_7">전체적용</label>
            </td>
        </tr>
        <tr>
            <th scope="row"><label for="it_6">개인별구메한도</label></th>
            <td colspan="2">
                <?php echo help("각 회원당 구매가능한 상품의 갯수를 지정하는 옵션으로, 숫자만 입력해주시면 됩니다.<br>상품이 회원만 구매가능할 때, 해당 기능을 제대로 사용할 수 있습니다."); ?>
                <input type="text" name="it_6" value="<?php echo $it['it_6']; ?>" id="it_6" class="frm_input">
            </td>
        </tr>
        <tr>
            ...
            <td colspan="2">
                ...
                <input type="text" name="ec_mall_pid" value="<?php echo get_text($it['ec_mall_pid']); ?>" id="ec_mall_pid" class="frm_input w100per">
            </td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td>
                ...
                <input type="text" name="it_sell_email" value="<?php echo $it['it_sell_email']; ?>" id="it_sell_email" class="frm_input w100per">
            </td>
            <td class="td_grpset">
...
<section id="anc_sitfrm_cost">
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        <tr>
            ...
            <td>
                <?php echo help("잠시 판매를 중단하거나 재고가 없을 경우에 체크해 놓으면 <span class=red>품절상품</span>으로 표시됩니다."); ?>
                <label class="switch-check">
                <input type="checkbox" name="it_soldout" value="1" id="it_soldout" <?php echo ($it['it_soldout']) ? "checked" : ""; ?>> 예
                <div class="check-slider round"></div>
                </label>
                <label for="it_soldout"><span class="blue font-14 font-bold">on</span> 으로 설정시 <b>품절로 표시됩니다</b></label>
            </td>
            <td class="td_grpset">
        ...
        <tr>
            ...
            <td colspan="2">
                ...
                <label for="it_stock_sms"><span class="blue font-14 font-bold">on</span> 으로 설정하면 <b>품절상품에 대해서 재입고시 알림</b></label>
            </td>
        </tr>
        <tr>
                ...
                <div class="sit_option tbl_frm01">
                    ...
                    <tr>
                        <th scope="row">
                            ...
                            <input type="text" name="opt1_subject" value="<?php echo $opt_subject[0]; ?>" id="opt1_subject" class="frm_input" size="10">
                        </th>
                        <td>
                            <label for="opt1"><b>옵션1 항목</b></label>
                            <input type="text" name="opt1" value="" id="opt1" class="frm_input w100per">
                        </td>
                    </tr>
                    <tr>
                        <th scope="row">
                            <label for="opt2_subject">옵션2</label>
                            <input type="text" name="opt2_subject" value="<?php echo $opt_subject[1]; ?>" id="opt2_subject" class="frm_input" size="10">
                        </th>
                        <td>
                            <label for="opt2"><b>옵션2 항목</b></label>
                            <input type="text" name="opt2" value="" id="opt2" class="frm_input w100per">
                        </td>
                    </tr>
                     <tr>
                        <th scope="row">
                            <label for="opt3_subject">옵션3</label>
                            <input type="text" name="opt3_subject" value="<?php echo $opt_subject[2]; ?>" id="opt3_subject" class="frm_input" size="10">
                        </th>
                        <td>
                            <label for="opt3"><b>옵션3 항목</b></label>
                            <input type="text" name="opt3" value="" id="opt3" class="frm_input w100per">
                        </td>
                    </tr>
                    </tbody>
                    </table>
                ...
                <div id="sit_supply_frm" class="sit_option tbl_frm01">
                    ...
                    <tr>
                        <th scope="row">
                            ...
                            <input type="text" name="spl_subject[]" id="spl_subject_<?php echo $seq; ?>" value="<?php echo $spl_subject[$i]; ?>" class="frm_input" size="10">
                        </th>
                        <td>
                            <label for="spl_item_<?php echo $seq; ?>"><b>추가<?php echo $seq; ?> 항목</b></label>
                            <input type="text" name="spl[]" id="spl_item_<?php echo $seq; ?>" value="" class="frm_input w100per">
                            <?php
                            if($i > 0)
        ...
        <tr>
            ...
            <td>
                ...
                <div id="limg<?php echo $i; ?>" class="banner_or_img_layer">
                    <img src="<?php echo G5_DATA_URL; ?>/item/<?php echo $it['it_img'.$i]; ?>" title="상품사진을 클릭하면 닫힙니다" width="<?php echo $size[0]; ?>" height="<?php echo $size[1]; ?>" class="sit_wimg_close">
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        <tr>
            ...
            <th scope="row" style="height:119px; background:#fafafa; padding:0; margin:0;">
                ...
                ?>
                <!-- 큰이미지보기/스크립트 -->
                <div id="limg<?php echo $i; ?>" class="banner_or_img_layer" style="margin-left:-300px;">
                    <img src="<?php echo G5_DATA_URL; ?>/item/<?php echo $it['it_img'.$i]; ?>" title="상품사진을 클릭하면 닫힙니다" width="<?php echo $size[0]; ?>" height="<?php echo $size[1]; ?>" class="sit_wimg_close">
                    <!--<br><button type="button" class="sit_wimg_close">닫기</button>-->
                </div>
                <!--//-->
    ...
    <div class="compare_wrap">
        <section class="compare_left">
            ...
            <span class="srel_pad">
                ...
                <input type="text" name="sch_name" id="sch_name" class="frm_input" size="10">
...
<section id="anc_sitfrm_relation" class="srel">
    <?php echo $pg_anchor; ?>
    <h2 class="h2_frm">관련리뷰</h2>
    <div class="local_desc02 local_desc">
        <p>
            등록된 전체상품 목록에서 상품분류를 선택하면 해당 상품 리스트가 연이어 나타납니다.<br>
            상품리스트에서 관련 리뷰로 추가하시면 선택된 관련상품 목록의 리뷰에도 <strong>함께</strong> 등록됩니다.<br>
            예를 들어, A 상품에 B 상품을 관련상품으로 등록하면 B 상품에도 A 상품의 리뷰가 등록되며, <strong>확인 버튼을 누르셔야 정상 반영됩니다.</strong>
        </p>
    </div>
    ...
    <div class="compare_wrap">
        <section class="compare_left">
            <h3>등록된 전체상품 목록</h3>
            <label for="use_sch_relation" class="sound_only">상품분류</label>
            <span class="srel_pad">
                <select id="use_sch_relation">
                    <option value=''>분류별 상품</option>
                    <?php
                        $sql = " select * from {$g5['g5_shop_category_table']} ";
                        if ($is_admin != 'super')
                ...
                <label for="sch_name" class="sound_only">상품명</label>
                <input type="text" name="use_sch_name" id="use_sch_name" class="frm_input" size="10">
                <button type="button" id="use_btn_search_item" class="btn_frmline">검색</button>
            </span>
            <div id="use_relation" class="srel_list">
                <p>상품의 분류를 선택하시거나 상품명을 입력하신 후 검색하여 주십시오.</p>
            </div>
            <script>
            $(function() {
                $("#use_btn_search_item").click(function() {
                    var use_ca_id = $("#use_sch_relation").val();
                    var use_it_name = $.trim($("#use_sch_name").val());
                    var $use_relation = $("#use_relation");

                    if(ca_id == "" && it_name == "") {
                        $use_relation.html("<p>상품의 분류를 선택하시거나 상품명을 입력하신 후 검색하여 주십시오.</p>");
                        return false;
                    }

                    $("#use_relation").load(
                        "./itemuseformrelation.php",
                        { it_id: "<?php echo $it_id; ?>", ca_id: use_ca_id, it_name: use_it_name }
                    );
                });

                $(document).on("click", "#use_relation .add_item", function() {
                    // 이미 등록된 상품인지 체크
                    var $use_li = $(this).closest("li");
                    var use_it_id = $use_li.find("input:hidden").val();
                    var use_it_id2;
                    var use_dup = false;
                    $("#use_reg_relation input[name='use_re_it_id[]']").each(function() {
                        use_it_id2 = $(this).val();
                        if(use_it_id == use_it_id2) {
                            use_dup = true;
                            return false;
                        }
                    });
                    ...
                    var use_cont = "<li>"+$use_li.html().replace("add_item", "del_item").replace("추가", "삭제")+"</li>";
                    var use_count = $("#use_reg_relation li").size();

                    if(use_count > 0) {
                        $("#use_reg_relation li:last").after(use_cont);
                    } else {
                        $("#use_reg_relation").html("<ul>"+use_cont+"</ul>");
                    }

                    $use_li.remove();
                });

                $(document).on("click", "#use_reg_relation .del_item", function() {
                    if(!confirm("상품을 삭제하시겠습니까?"))
                        return false;
                    ...
                    var use_count = $("#use_reg_relation li").size();
                    if(use_count < 1)
                        $("#use_reg_relation").html("<p>선택된 상품이 없습니다.</p>");
                });
            });
            </script>
        </section>
        ...
        <section class="compare_right">
            <h3>선택된 관련상품 목록</h3>
            <span class="srel_pad"></span>
            <div id="use_reg_relation" class="srel_sel">
                <?php
                $use_str = array();
                $sql = " select b.ca_id, b.it_id, b.it_name, b.it_price
                           from {$g5['g5_shop_item_use_relation_table']} a
                           left join {$g5['g5_shop_item_table']} b on (a.it_id2=b.it_id)
                          where a.it_id = '$it_id'
                          order by iur_id asc ";
                $result = sql_query($sql);
                ...
                ?>
                    <li>
                        <input type="hidden" name="use_re_it_id[]" value="<?php echo $row['it_id']; ?>">
                        <div class="list_item"><?php echo $it_name; ?></div>
                        <div class="list_item_btn"><button type="button" class="del_item btn_frmline">삭제</button></div>
                    </li>
                <?php
            ...
            <input type="hidden" name="use_it_list" value="<?php echo $use_str; ?>">
        </section>
...
<div id="bq_right1">   
    ...
    <a href="<?php echo G5_SHOP_URL ?>/item.php?it_id=<?php echo $it_id ?>" target="_blank"><i class="fas fa-laptop"></i> 상품보기</a>
    </div>
    <?php } ?>
    
    <div class="bq_basic">
    <a href="./itemlist.php?<?php echo $qstr ?>"><i class="fas fa-tasks"></i> 목록</a>
    </div>
...
function fitemformcheck(f)
{
    ...
    // 관련리뷰처리
    var use_item = new Array();
    var use_re_item = use_it_id = "";
    ...
    $("#use_reg_relation input[name='use_re_it_id[]']").each(function() {
        ...
        use_item.push(use_it_id);
    });

    if(use_item.length > 0)
        use_re_item = use_item.join();

    $("input[name=use_it_list]").val(use_re_item);
