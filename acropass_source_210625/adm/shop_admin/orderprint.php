...
<section><!-- 주문 성세내역출력 섹션 열기 { -->
<h5><i class="fa fa-download"></i> 주문 상세내역 출력(고객별 총 주문합계 포함)</h5>
<div class="local_sch02 local_sch" style="border:solid 5px #aaa; background:#fff; padding:25px 25px 25px;"><!-- 주문 랭킹내역 출력 선택창 열기 { -->

    <div>
        <form name="forderprint" action="./orderprintresult4.php" onsubmit="return forderprintcheck(this);" autocomplete="off">
        <input type="hidden" name="case" value="5">

        <strong class="sch_long">기간별 출력</strong>
        <input type="radio" name="csv" value="xls" id="xls6">
        <label for="xls6">MS엑셀 XLS 파일</label>
        <label for="ct_status_p" class="sound_only">출력대상</label>
        <select name="ct_status" id="ct_status_p">
            <option value="주문">주문</option>
            <option value="입금">입금</option>
        ...
        <label for="fr_date4" class="sound_only">기간 시작일</label>
        <input type="text" name="fr_date4" value="<?php echo date("Ymd"); ?>" id="fr_date4" required class="required frm_input" size="8" maxlength="8">
        ~
        <label for="to_date4" class="sound_only">기간 종료일</label>
        <input type="text" name="to_date4" value="<?php echo date("Ymd"); ?>" id="to_date4" required class="required frm_input" size="8" maxlength="8">
        <input type="submit" value="출력 (새창)" class="btn_submit">
...
<script>
$(function(){
    $("#fr_date, #to_date, #fr_date2, #to_date2, #fr_date3, #to_date3, #fr_date4, #to_date4").datepicker({ changeMonth: true, changeYear: true, dateFormat: "yymmdd", showButtonPanel: true, yearRange: "c-99:c+99", maxDate: "+0d" });
});
